...
launch_machine() {
    ...
    local command="cd '$REPO_ROOT' && HOSTFS_ROOT=$AE_HOSTFS_ROOT_SHELL CPU_NUM=$GUEST_CPU_NUM MACHINE_NUM=$NUM_MACHINES ./build/simulate.sh $machine 2>&1 | tee '$logfile'"
...
start_cluster() {
    ...
    stop_cluster
    make clean-dsm-meta
    ae_start_log_watchdog "$NUM_MACHINES" "$label"
