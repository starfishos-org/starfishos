...
MU_TEST(test_multi_buddy)
{
	...
	/* allocate the page_metadata area */
	size = npages * sizeof(struct page);
	start = mmap((void *)0x50100000000, size, PROT_READ|PROT_WRITE,
		     MAP_SHARED|MAP_ANONYMOUS, -1, 0);
	/* allocate the physical memory area */
	size = npages * (0x1000);
	start_addr = (u64)mmap((void *)0x60100000000, size, PROT_READ|PROT_WRITE,
		     MAP_SHARED|MAP_ANONYMOUS, -1, 0);
	init_buddy(&global_mem[1], start, start_addr, npages, NVM_PAGE);
