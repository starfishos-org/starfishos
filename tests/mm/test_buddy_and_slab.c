...
static unsigned long buddy_num_free_page(struct phys_mem_pool *zone)
{
        ...
        for (i = 0; i < BUDDY_MAX_ORDER; ++i) {
                //printf("free list order: %ld contains %ld\n", i,
                //	zone->free_lists[i].nr_free);
...
{
        mu_assert((idx < zone->pool_phys_page_num) && (idx >= 0), "invalid page idx");
}
...
void test_buddy(void)
{
        ...
        size = npages * (0x1000);
        start_addr = (u64)mmap((void *)0x60000000000, size, PROT_READ|PROT_WRITE,
                        MAP_SHARED|MAP_ANONYMOUS, -1, 0);

        init_buddy(&global_mem[0], start, start_addr, npages, NVM_PAGE);
        ...
        test_alloc(&global_mem[0], npages/powl(2, BUDDY_MAX_ORDER - 1),
                        BUDDY_MAX_ORDER - 1);
...
static void check_slab_range(struct slab_header *slab, void *addr, int cnt)
{
        ...
        if (!((addr > (void*)slab) && (addr < (void*)slab + SIZE_OF_ONE_SLAB))) {
                printf("\n%s fail: cnt: %d, slab addr: 0x%lx, slot addr: 0x%lx\n",
                                __func__, cnt, (long)slab, (long)addr);
                exit(-1);
        }
        mu_check((void *)addr > (void*)slab);
        mu_check((void *)addr < ((void*)slab + SIZE_OF_ONE_SLAB));
}
...
{
        printf("slab_pool[%d] current_slab: %p\n", order, slab_pool[order].current_slab);
...
void test_slab(void)
{
        ...
        ptr = malloc(SIZE_OF_ONE_SLAB / powl(2, SLAB_MIN_ORDER) * sizeof(void*));
        mu_check(ptr != NULL);
        ...
        for (i = SLAB_MIN_ORDER; i <= SLAB_MAX_ORDER; ++i) {
                ...
                for (j = 0; j < cnt; ++j) {
                        //printf("test: slab %d alloc/free alloc round %d\n", i, j);
        ...
        for (i = SLAB_MIN_ORDER; i <= SLAB_MAX_ORDER; ++i) {
                ...
                for (j = 0; j < cnt; ++j) {
                        ...
                        mu_check(ptr[j] != NULL);
                        *(int *)ptr[j] = j;
                        /* This check cannot hold since the slab will choose
                         * one slab from the partial list as the current one.
                         */
                        // check_slab_range(slab_pool[i].current_slab, ptr[j], j);
                }
