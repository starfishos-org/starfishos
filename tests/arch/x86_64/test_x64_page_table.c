...
void test_page_mappings(void *pgtbl, paddr_t *pas,
...
MU_TEST(test_map_unmap_page)
{
	...
	err = map_range_in_pgtbl(pgtbl, va, 0x100000, PAGE_SIZE, DEFAULT_FLAGS);
	mu_assert_int_eq(0, err);
	...
	err = unmap_range_in_pgtbl(pgtbl, va, PAGE_SIZE);
	mu_assert_int_eq(0, err);
...
rerand:
		...
		err = map_range_in_pgtbl(pgtbl, vas[i], pas[i], PAGE_SIZE,
					 DEFAULT_FLAGS);
		mu_assert_int_eq(0, err);
	}
	...
	for (i = 0; i < RND_MAPPING_PAGES; i++) {
		...
		err = unmap_range_in_pgtbl(pgtbl, vas[i], PAGE_SIZE);
		mu_assert_int_eq(0, err);
		vas[i] = 0;
		pas[i] = 0;
	}
