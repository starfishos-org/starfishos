 ...
 * foreground thread, it will call waitpid to wait the thread to exit or suspend.
 *
 ...
 * shell.
 *
 * Maybe someone will extend the function of this shell in the future. Please add
 * buildin command in buildin_cmd.c and add other extra function in a dependent
 * file.
 */
...
static int shell_ipc_server_cap = -1;
...
char *readline(const char *prompt)
{
	...
	handle_new_char:
		if (c < 0) {
			 ...
			 */
			strlcpy(buf + index - 1, buf + index, BUFLEN - index + 1);
			buf[i] = 0;
			...
			/* Redisplay the characters after the newly entered character. */
			for (j = index - 1; j < i; j++) {
				putchar(buf[j]);
			}
			...
			if (ch == '[') {
				...
				case 'A':
					...
					if (do_up()) {
						...
						buf[j] = 0;
						i = (int)strlen(history_cmd_pointer
								   ->cmd);
						index = i;
					}
					break;
...
int main(void)
{
	...
	handle_wait:
		...
		if (waitchild) {
			/* FIXME: only wait for child process when `waitchild` is on,
			   I think this should only be a temporary solution, we need
			   full-featured job control in the future. */
			pid_t pid = ret;
			int status;
			...
			if (fg_job.pmo_cap > 0) {
				chcore_auto_unmap_pmo(fg_job.pmo_cap,
						      (u64)foreground_pmo_addr,
						      PAGE_SIZE);
			}
			fg_job.pmo_cap = -1;
