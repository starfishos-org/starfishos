...
int fat_open(char *path, int flags, int mode, ino_t *vnode_id, size_t *vnode_size, int *vnode_type, void **vnode_private)
{
	int ff_mode;
	int ret = 0;
	FIL *fil;
...
int fat_close(void *operator, bool is_dir)
{
	int ret;
	if (is_dir) {
...
int fat_ftruncate(void *operator, size_t len)
{
	int ret = 0;
	FIL *fp;
...
{
	int partition = fr->mount.paritition; // use fd field to store "sd partition"
