 ...
 * 	- FAT32 has no hard link, so there is no pair of paths points to one file inode
 * 	- When open a path as FIL struct, we have `dir_sect` and `dir_ptr` fields
 * 		`dir_sect` stands the sect number of dentry placed
 * 		`dir_ptr` is a pointer in win[] buffer in FATFS struct
 * 		so (`dir_sect` * SECT_SIZE + (`dir_ptr` - win[])) is inode location in disk
 * For direcories: we use the first cluster number of block chain PLUS ONE as ino
 ...
 * 	- Why PLUS ONE:
 * 		The first dentry in the dir and the dir chain beginning have same location.
 * 		But thanks to the alignment of dentry, file's ino is always end with 0
 * 		We use the last bit to make a distinction between dir and file ino
 */

#define FAT_WINBASE (uint64_t)fs->win
#define FP_DENTRY_WIN_OFFSET(fp) (uint64_t)(fp)->dir_ptr-FAT_WINBASE
#define FILE_INO(fp) (ino_t)(((uint32_t)(fp)->dir_sect)<<10)+(uint32_t)FP_DENTRY_WIN_OFFSET(fp)
