...
#define page_cache_debug(fmt, ...) printf(COLOR_YELLOW"<%s:%s:%d>: "COLOR_DEFAULT" "fmt, __FILE__, __func__, __LINE__, ##__VA_ARGS__)
#else
#define page_cache_debug(fmt, ...) do {} while (0)
#endif
...
struct page_cache_entity_of_inode {
	...
	/* Used for easily traversing all pages and quickly finding a specific page. */
...
typedef int (*file_reader_t)(char *buf, pidx_t file_page_idx, void *private_data);

/* Write a specific block or page(when page_block_idx == -1) to file. */
typedef int (*file_writer_t)(char *buf, pidx_t file_page_idx, int page_block_idx, void *private_data);
...
struct user_defined_funcs {
	 ...
	 * User may do something when pce turns empty to non-empty (combined with a private)
	 */
 ...
 */
void fs_page_cache_init(PAGE_CACHE_STRATEGY strategy, struct user_defined_funcs *user_func);

/*
 * Alloc a new page_cache_entity_of_inode.
 */
struct page_cache_entity_of_inode *new_page_cache_entity_of_inode(ino_t host_idx, void *private_data);
 ...
 */
int page_cache_check_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx);
 ...
 */
char *page_cache_get_block_or_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx, int page_block_idx, PAGE_CACHE_OPERATION_TYPE op_type);

/*
 * Put a block or a page to corresponding page cache.
 */
void page_cache_put_block_or_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx, int page_block_idx, PAGE_CACHE_OPERATION_TYPE op_type);
 ...
 *	 if failed, return -1.
 */
int page_cache_flush_block_or_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx, int page_block_idx);
 ...
 *	 if failed, return -1.
 */
int page_cache_pin_single_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx);
 ...
 *	 if failed, return -1.
 */
int page_cache_unpin_single_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx);
 ...
 *	 if failed, return -1.
 */
int page_cache_pin_multiple_pages(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx, u64 page_num);
 ...
 *	 if failed, return -1.
 */
int page_cache_unpin_multiple_pages(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx, u64 page_num);
 ...
 *	 if failed, return -1.
 */
int page_cache_evict_single_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx);
