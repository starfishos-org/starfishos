...
static void flush_and_free_page(struct cached_page *p)
{
	...
	/* pages with empty contents is marked not dirty. So it won't be flushed. */
...
/*
 * Boost a node in inactive list. Remove the node and append it to the tail of the active list.
 */
static inline void boost_inactive_page(struct cached_page *p)
{
	...
	if (page_cache.active_list.size > ACTIVE_LIST_MAX) {
		/* If active list is full, move head unpinned page to inactive list. */
		head_page = cached_pages_list_top_node(&page_cache.active_list, ACTIVE_LIST);
		cached_pages_list_delete_node(&page_cache.active_list, head_page, ACTIVE_LIST);
		cached_pages_list_append_node(&page_cache.inactive_list, head_page, INACTIVE_LIST);

		if (page_cache.inactive_list.size > INACTIVE_LIST_MAX) {
			/* if inactive list is full, evict head unpinned page. */
...
/* The caller should hold per page_cache mutex lock to protect list operations! */
...
/*
 * Find a page from active_list, inactive_list and pinned_pages_list, if page not found,
 * create a new one and insert it to inactive list and pages list.
				     ...
				     pidx_t file_page_idx, bool *is_new)
{
	...
	if (p == NULL) {
...
#endif
		/* Cache miss, create a new page and fill it with corresponding contents. */
		page_cache_debug(
		...
		/* According to two list strategy, a new page should be inserted into inactive list. */
		cached_pages_list_append_node(&page_cache.inactive_list, p, INACTIVE_LIST);
		if (page_cache.inactive_list.size > INACTIVE_LIST_MAX) {
			/* if inactive list is full, evict head unpinned page. */
			evict_head_page(&page_cache.inactive_list,
					INACTIVE_LIST);
		...
		page_cache.user_func.file_read(p->content, file_page_idx, pce->private_data);
...
void *write_back_routine(void *args)
{
	...
	while (1) {
		...
		if (page_cache.cache_strategy == WRITE_BACK) {
			page_cache_debug("[write_back_routine] write back routine started.\n");
...
void fs_page_cache_init(PAGE_CACHE_STRATEGY strategy, struct user_defined_funcs *uf)
{
	pthread_t thread;
				  ...
				  PAGE_CACHE_OPERATION_TYPE op_type)
{
	...
	case DIRECT:
		 ...
		 * There won't be any use-after-free error because page_cache_lock protect us from this error.
		 */
		flush_and_free_page(p);
		break;
...
int page_cache_flush_all_pages(void)
{
	...
	page_cache_debug("[page_cache_flush_all_pages] flush all pages finished.\n");
...
int page_cache_delete_single_page(struct page_cache_entity_of_inode *pce, pidx_t file_page_idx)
{
	struct cached_page *p;
	int ret = 0;
...
int fake_file_reader(char *a, pidx_t b, void *c)
{
	// page_cache_debug("read page %d block %d into %p\n", b, c, a);
	return 1;
}

int fake_file_writer(char *a, pidx_t b, int c, void *d)
{
...
void print_all_list(void)
{
	...
	page_cache_debug("[page_cache INFO] starts\n");

	page_cache_debug(
		"[page_cache INFO] active_list contains %d elements:\n",
		page_cache.active_list.size);
	page_cache_debug("list info:\n");
	for_each_in_list (p,
			  struct cached_page,
			  two_list_node,
			  &page_cache.active_list.queue)
		page_cache_debug("%d:%d  , dirty flag = %d\n",
				 p->owner->host_idx,
				 p->file_page_idx,
				 is_block_or_page_dirty(p, -1));

	page_cache_debug(
		"[page_cache INFO] inactive_list contains %d elements:\n",
		page_cache.inactive_list.size);
	for_each_in_list (p,
			  struct cached_page,
			  two_list_node,
			  &page_cache.inactive_list.queue)
	...
	page_cache_debug(
		"[page_cache INFO] pinned_pages_list contains %d elements:\n",
		page_cache.pinned_pages_list.size);
	page_cache_debug("list info:\n");
	for_each_in_list (p,
			  struct cached_page,
			  pinned_pages_node,
	...
	page_cache_debug("[page_cache INFO] ends\n");
}

void print_pce(struct page_cache_entity_of_inode *pce)
{
	struct cached_page *p;

	page_cache_debug("print pce start\n");
	/* Find all target pages. */
	for_each_in_list (
		p, struct cached_page, inode_pages_node, &pce->pages.queue) {
		page_cache_debug("%d:%d in list %d\n",
				 p->owner->host_idx,
				 p->file_page_idx,
				 p->in_which_list);
	}

	page_cache_debug("print pce end\n");
}

void test_fs_page_cache(void)
{
	struct page_cache_entity_of_inode *pce1, *pce2;
	char *buf;

	page_cache_debug("[test_fs_page_cache] test started!\n");

	fs_page_cache_init(WRITE_THROUGH, fake_file_reader, fake_file_writer);

	pce1 = new_page_cache_entity_of_inode(0, NULL);
	pce2 = new_page_cache_entity_of_inode(1, NULL);

	/* Max page test. */
	for (int i = 0; i < ACTIVE_LIST_MAX; ++i) {
		page_cache_debug(
			"----------------------evict test start---------------------\n");
		buf = page_cache_get_block_or_page(pce1, i, 5, READ);
		if (!page_cache_check_page(pce1, i))
			BUG("check page failed.\n");
		page_cache_put_block_or_page(pce1, i, 5, READ);

		buf = page_cache_get_block_or_page(pce2, i, 5, READ);
		if (!page_cache_check_page(pce2, i))
			BUG("check page failed.\n");
		page_cache_put_block_or_page(pce2, i, 5, READ);

		/* Now 2 elements should be in inactive_list. */
		print_all_list();

		page_cache_evict_single_page(pce1, i);
		page_cache_evict_single_page(pce2, i);

		/* Now 2 elements should be disappeared. */
		print_all_list();
		...
		page_cache_evict_pages_of_inode(pce1);
		page_cache_evict_pages_of_inode(pce2);

		/* Now all elements should be disappeared. */
		print_all_list();
		...
		page_cache_debug(
			"---------------------flush test start---------------------\n");
		page_cache_switch_strategy(WRITE_BACK);
		buf = page_cache_get_block_or_page(pce1, i, 5, WRITE);
		if (!page_cache_check_page(pce1, i))
			BUG("check page failed.\n");
		page_cache_put_block_or_page(pce1, i, 5, WRITE);

		buf = page_cache_get_block_or_page(pce2, i, 5, WRITE);
		if (!page_cache_check_page(pce2, i))
			BUG("check page failed.\n");
		page_cache_put_block_or_page(pce2, i, 5, WRITE);

		/* Now 2 elements should be dirty. */
		print_all_list();

		page_cache_flush_block_or_page(pce1, i, 5);
		page_cache_flush_block_or_page(pce2, i, 5);

		/* Now 2 elements should be clean. */
		print_all_list();
		...
		page_cache_flush_pages_of_inode(pce1);
		page_cache_flush_pages_of_inode(pce2);
		...
		page_cache_debug(
			"---------------------pin test start---------------------\n");
		page_cache_pin_single_page(pce1, i);
		page_cache_pin_single_page(pce2, i);

		/* Now 2 elements should be in pinned_pages_list. */
		print_all_list();

		page_cache_unpin_single_page(pce1, i);
		page_cache_unpin_single_page(pce2, i);

		/* Now 2 elements should be disappeared. */
		print_all_list();

		page_cache_pin_multiple_pages(pce1, i, 2);
		page_cache_pin_multiple_pages(pce2, i, 2);

		/* Now 4 elements should in pinned_pages_list. */
		print_all_list();

		page_cache_unpin_multiple_pages(pce1, i, 2);
		page_cache_unpin_multiple_pages(pce2, i, 2);

		/* Now 4 elements should be disappeared. */
		print_all_list();
		...
		page_cache_switch_strategy(WRITE_THROUGH);

		/* Now 2 elements should be clean. */
		print_all_list();
		page_cache_debug(
			"---------------------switch strategy test end---------------------\n");

		page_cache_debug(
			"---------------------delete test start---------------------\n");
		page_cache_delete_single_page(pce1, i);

		/* Now 1 elements should be disappeared. */
		print_all_list();

		page_cache_delete_pages_of_inode(pce2);
	...
	/* Now there should be 2 clean pages in inactive list. */
	print_all_list();

	page_cache_debug("buf = %p\n", buf);
	page_cache_debug("[test_fs_page_cache] test finished!\n");
}
