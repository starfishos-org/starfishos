 ...
 */
int ext4_srv_open(char *path, int flags, int mode, ino_t *vnode_id, size_t *vnode_size, int *vnode_type, void **vnode_private)
{
	int r;
	int type;
	ext4_dir *dir;
...
int ext4_srv_close(void *operator, bool is_dir)
{
	int ret;
...
ssize_t ext4_srv_write(void *operator, off_t offset, size_t size, const char *buf)
{
	int ret;
	size_t wcnt = 0;
	ext4_file *fp = (ext4_file *)operator;
...
/* ftruncate */
int ext4_srv_ftruncate(void *operator, size_t len)
{
	int ret;
	ext4_file *fp;
	...
	if (original_size < len) {
		...
		while (original_size < len) {
			tmp_target =
				MIN(len, ROUND_UP(original_size + 1, EXT4_INODE_BLOCK_SIZE));
			ext4_fwrite(fp, zero_block, tmp_target - original_size, &wcnt);
