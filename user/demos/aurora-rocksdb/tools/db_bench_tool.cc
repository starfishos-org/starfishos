...
/*
 * FN: currently chcore can not pass two long args, so this file we specify how
 * each args should be for our tests; the original file is db_bench_tool_default.cc
 */
...
DEFINE_int32(key_size, 48, "size of each key");
...
DEFINE_int64(cache_size, 256 << 20,  // 8 << 20 => 8MB
...
DEFINE_double(iter_sigma, 14.236,
              ...
              "f(x)=(1/sigma)*(1+k*(x-theta)/sigma)^-(1/k+1)");
DEFINE_double(mix_get_ratio, 0.83,
              "The ratio of Get queries of mix_graph workload");
DEFINE_double(mix_put_ratio, 0.14,
              "The ratio of Put queries of mix_graph workload");
DEFINE_double(mix_seek_ratio, 0.03,
 ...
 private:
  ...
  void Open(Options* opts) {
    if (!InitializeOptionsFromFile(opts)) {
      ...
      printf("Initializing RocksDB Options from command-line flags finished\n");
    }

    InitializeOptionsGeneral(opts);
    printf("InitializeOptionsGeneral finished\n");
  }
