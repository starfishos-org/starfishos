...
## Chcore-support
To support it run on chcore, we have disable several flags:
- fallocate (set WITH_FALLOCATE OFF)
- sync_file_range_write (change the CMakefileList)
- fadvise (change the code: #ifdef OS_LINUX => #ifdef ROCKSDB_FADVISE_PRESENT)
- readahead (change the code: #ifdef OS_LINUX => #ifdef ROCKSDB_READAHEAD_PRESENT)
