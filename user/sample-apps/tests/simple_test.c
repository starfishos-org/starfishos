...
__attribute__((optimize("O0"))) void calculate(int size) {
  uint64_t arr[size];
  arr[0] = 1;
  arr[1] = 2;
...
int main(int argc, char **argv) {
  if (argc != 4) {
    printf("Usage: %s <iter_count> <size> <num_threads>\n", argv[0]);
    return 1;
  }
  int iter_count = atoi(argv[1]);
  int size = atoi(argv[2]);
  int num_threads = atoi(argv[3]);
  printf("iter_count: %d, size: %d, num_threads: %d\n", iter_count, size,
         num_threads);
