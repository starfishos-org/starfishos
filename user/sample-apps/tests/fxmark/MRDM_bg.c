...
{
	struct fx_opt *fx_opt = fx_opt_worker(worker);
	sprintf(test_file, "%s/n_shdir_rd_bg-%d-%" PRIu64 ".dat",
...
static int bg_work(struct worker *worker)
{
	...
	while (!bench->stop) {
		/* delete a randomly selected file in the shared directory */
...
struct bench_operations n_shdir_rd_bg_ops = {
	.pre_work  = pre_work, 
	.main_work = main_work,
};
