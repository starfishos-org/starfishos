...
char **argv;
{
        ...
        if ((buf = (char *)malloc(buflen + bufalign)) == (char *)NULL)
                err("malloc");
        if (bufalign != 0)
                buf += (bufalign - ((long)buf % bufalign) + bufoffset)
                       % bufalign;
        ...
        if (!udp) {
                ...
                if (trans) {
                        ...
                        if (options) {
#if defined(BSD42)
                                if (setsockopt(fd, SOL_SOCKET, options, 0, 0)
                                    < 0)
#else /* BSD43 */
                                if (setsockopt(fd,
                                               SOL_SOCKET,
...
int pattern(cp, cnt) register char *cp;
register int cnt;
{
        register char c;
        c = 0;
...
#if defined(SYSV)
/*ARGSUSED*/ static getrusage(ignored, ru) int ignored;
...
/*ARGSUSED*/ static gettimeofday(tp, zp) struct timeval *tp;
struct timezone *zp;
{
        tp->tv_sec = time(0);
        tp->tv_usec = 0;
 ...
 *
 */
double read_timer(str, len) char *str;
int len;
{
...
static void prusage(r0, r1, e, b, outp) register struct rusage *r0, *r1;
struct timeval *e, *b;
char *outp;
{
...
static void tvadd(tsum, t0, t1) struct timeval *tsum, *t0, *t1;
{
...
static void tvsub(tdiff, t1, t0) struct timeval *tdiff, *t1, *t0;
{
...
int count;
{
        ...
        if (udp) {
                cnt = recvfrom(
                        fd, buf, count, 0, (struct sockaddr *)&from, &len);
                numCalls++;
        } else {
                if (b_flag)
 ...
 *			N W R I T E
 */
int Nwrite(fd, buf, count) int fd;
void *buf;
int count;
{
        register int cnt;
