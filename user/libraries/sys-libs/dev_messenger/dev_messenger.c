...
struct dev_desc *dm_open(const char *devname, int wait)
{
	...
	/* Prepare ipc_msg */
	dev_ipc_msg = ipc_create_msg(devmgr_ipc_struct, sizeof(request), 0);
...
int dm_ctrl(struct dev_desc *dev_desc, u64 request, void *data, int len)
{
	...
	/* Check connection */
	if (devmgr_ipc_struct == NULL) {
		/* devmgr_ipc_struct cannot be NULL if any device has been opened */
...
int dm_list_dev(struct dev_info **info)
{
	...
	devnum = 20;
	dev_ipc_msg =
		ipc_create_msg(devmgr_ipc_struct,
			       sizeof(u32) + devnum * sizeof(struct dev_info),
			       0);
		     ...
		     search_selector selector)
{
	...
	case SEARCH_DEV_NAME:
		for (i = 0; i < num; ++i) {
			if (strncmp(dev_info[i].devname, string, DEV_NAME_LEN) == 0) {
				return 0;
			}
		}
		break;
	case SEARCH_VENDOR:
		for (i = 0; i < num; ++i) {
			if (strncmp(dev_info[i].vendor, string, DEV_NAME_LEN) == 0) {
				return 0;
			}
		}
		break;
