...
static int serial_write(struct dev_desc *dev_desc, void *data, int datalen)
{
	...
	max_len = IPC_PER_SHM_SIZE - sizeof(devmgr_request) - sizeof(ipc_msg_t);
	if (datalen > max_len) {
		// TODO: use share memory to support large data.
		// ipc_msg = ipc_create_msg(
		// 	devmgr_ipc_struct, sizeof(devmgr_request), 0);
		// ipc_set_msg_data(
		// 	ipc_msg, &devmgr_request, 0, sizeof(devmgr_request));
		return 0;
	} else {
		ipc_msg = ipc_create_msg(
			devmgr_ipc_struct, sizeof(devmgr_request) + datalen, 0);
		ipc_set_msg_data(
			ipc_msg, &devmgr_request, 0, sizeof(devmgr_request));
		ipc_set_msg_data(
...
static int serial_open(struct dev_desc *dev_desc)
{
	...
	ipc_msg = ipc_create_msg(devmgr_ipc_struct, sizeof(devmgr_request), 0);
