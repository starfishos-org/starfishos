...
static int clip_hline_with_rect(const RECT *rc, int l, int r, int y,
	int *cl, int *cr)
{
...
/* Return -1 if line out of rect */
static int clip_vline_with_rect(const RECT *rc, int x, int t, int b,
	int *ct, int *cb)
{
...
int _draw_line(const PDC dc, int x1, int y1, int x2, int y2)
{
	...
	set_rect(&rc, 0, 0, dc_width(dc) - 1, dc_height(dc) - 1);
	if (clip_line_with_rect(&rc, x1, y1, x2, y2, &cx1, &cy1, &cx2, &cy2) != 0)
		return -1;
	...
	if (dc_clip_rgn(dc) == NULL)
		...
		for (i = 0; i < rgn_rc_num(dc_clip_rgn(dc)); ++i) {
			if (clip_line_with_rect(rgn_rc(dc_clip_rgn(dc), i),
				cx1, cy1, cx2, cy2, &cx3, &cy3, &cx4, &cy4) == 0)
