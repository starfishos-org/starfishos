...
static inline void init_gdi_rgn_diff()
{
	if (rgn_d0 == NULL)
		rgn_d0 = _create_rgn(DEFAULT_RGN_SIZE);
	else _clear_rgn(rgn_d0);
	if (rgn_d1 == NULL)
		rgn_d1 = _create_rgn(DEFAULT_RGN_SIZE);
	else _clear_rgn(rgn_d1);
}

static inline void init_gdi_rgn_comb()
{
	if (rgn_c0 == NULL)
		rgn_c0 = _create_rgn(DEFAULT_RGN_SIZE);
	else _clear_rgn(rgn_c0);
	if (rgn_c1 == NULL)
		rgn_c1 = _create_rgn(DEFAULT_RGN_SIZE);
	else _clear_rgn(rgn_c1);
}
...
static void rgn_push_rgn(PRGN dst, const PRGN src)
{
	...
	if (dst->rc_num + src->rc_num > dst->size) {
		dst->size = get_expand_size(dst->size, dst->rc_num + src->rc_num);
...
inline void init_gdi_rgn_ex()
{
	if (rgn_x0 == NULL)
		rgn_x0 = _create_rgn(DEFAULT_RGN_SIZE);
	else _clear_rgn(rgn_x0);
	if (rgn_x1 == NULL)
		rgn_x1 = _create_rgn(DEFAULT_RGN_SIZE);
	else _clear_rgn(rgn_x1);
}

inline PRGN get_gdi_rgn_ex0()
{
	return rgn_x0;
}

inline PRGN get_gdi_rgn_ex1()
{
	return rgn_x1;
}
