...
static inline void init_local_dc()
{
	if (local_dc == NULL)
...
static void do_bitblt_rect(const PDC dst, int x1, int y1,
	const PDC src, int x2, int y2, 
	int w, int h, enum BLT_OP bop, void *arg)
{
	...
	if (bop == BLT_RASTER) {
		...
		case ROP_SRCERASE:
			...
			break;
		case ROP_SRCINVERT:
			bitblt_srcinvert(dst, dst_ptr, w * bpp, h, src, src_ptr);
			break;
		case ROP_SRCPAINT:
	...
	int pmo_cap)
{
	...
	bitmap->raw_image = (u8 *)chcore_auto_map_pmo(pmo_cap, pitch * h,
		VM_READ | VM_WRITE);
	return bitmap;
}
...
{
	if (bitmap->pmo_cap != -1) {
		chcore_auto_unmap_pmo(bitmap->pmo_cap, (u64)bitmap->raw_image,
	...
	int w, int h, enum BLT_OP bop, void *arg)
{
	...
	for (i = 0; i < rgn_rc_num(rgn0); ++i) {
		rc = rgn_rc(rgn0, i);
		do_bitblt_rect(dst, rc->l, rc->t,
			src, rc->l - offx, rc->t - offy,
			rc->r - rc->l + 1, rc->b - rc->t + 1,
			bop, arg);
	}
...
int do_stretch_blt(const PDC dst, int x1, int y1, int w1, int h1,
	const PDC src, int x2, int y2, int w2, int h2,
	enum BLT_OP bop, void *arg)
{
	PRGN rgn0, rgn1;
	int i;
	const RECT *rc;
	...
	for (i = 0; i < rgn_rc_num(rgn0); ++i) {
		...
		dc_gfx(dst)->stretch_blt(dst, ax1, ay1, sx1, sy1,
			src, ax2, ay2, sx2, sy2,
			px, py, pw, ph, bop, arg);
	}
...
/* Bit-block transfer */
inline int _bitblt(const PDC dst, int x1, int y1, int w, int h,
...
{
	if (w1 == w2 && h1 == h2)
		return do_bitblt(dst, x1, y1, src, x2, y2, w1, h1,
			BLT_RASTER, &rop);
	else
		return do_stretch_blt(dst, x1, y1, w1, h1, src, x2, y2, w2, h2,
			BLT_RASTER, &rop);
}
