...
inline int draw_arc(const PDC dc, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
{
	if (dc == NULL)
		return -1;
...
{
	if (dst == NULL || dc_bitmap(dst) == NULL
		|| src == NULL || dc_bitmap(src) == NULL)
		return -1;
	...
	const PBITMAP bitmap, int x2, int y2, int w2, int h2, enum ROP rop)
{
	if (dst == NULL || dc_bitmap(dst) == NULL || bitmap == NULL)
		return -1;
	return _stretch_bitmap(dst, x1, y1, w1, h1, bitmap, x2, y2, w2, h2, rop);
}
	...
	const PDC src, int x2, int y2, int w2, int h2, u32 color)
{
	if (dst == NULL || dc_bitmap(dst) == NULL
		|| src == NULL || dc_bitmap(src) == NULL)
		return -1;
	return _transparent_blt(dst, x1, y1, w1, h1, src, x2, y2, w2, h2, color);
}
...
inline int blend(const PDC dst, int x1, int y1, int w1, int h1,
...
inline int combine_region(PRGN dst, const PRGN src1, const PRGN src2, enum RGN_COMB_MODE mode)
{
