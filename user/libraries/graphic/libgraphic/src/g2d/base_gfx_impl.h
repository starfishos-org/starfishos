...
#define template_bitblt(type, dst_dc, x1, y1, src_dc, x2, y2, \
	w, h, op, arg) { \
	type *_dst_ptr = (type *)get_dc_wr_ptr(dst_dc, x1, y1); \
	type *_src_ptr = (type *)get_dc_wr_ptr(src_dc, x2, y2); \
	int _y; \
	for (_y = 0; _y < h; ++_y) { \
		template_memop(type, _dst, _src, w, op, arg); \
 ...
 * : sx1 < sx2
 */
#define template_strecth_line_1(type, _dst_ptr, sx1, _src_ptr, sx2, \
	_src_xend_ptr, px, pw, op, arg) \
{ \
	type *_dst = _dst_ptr; \
	type *_src = _src_ptr; \
	int _qx1 = px; \
 ...
 * : sx1 >= sx2
 */
#define template_strecth_line_2(type, dst_ptr, sx1, src_ptr, sx2, \
	src_xend_ptr, px, pw, op, arg) \
{ \
	type *_dst = dst_ptr; \
	type *_src = src_ptr; \
	int _qx1 = px; \
 ...
 * : sy1 < sy2, sx1 < sx2
 */
#define template_strecth_rect_1(type, dst_dc, x1, y1, sx1, sy1, \
	src_dc, x2, y2, sx2, sy2, \
	px, py, pw, ph, op, arg) \
{ \
	type *_dst_ptr = (type *)get_dc_wr_ptr(dst_dc, x1, y1); \
	type *_src_ptr = (type *)get_dc_wr_ptr(src_dc, x2, y2); \
	type *_src_xend_ptr = (type *)get_dc_line_right_ptr(src_dc, y2); \
	type *_src_yend_ptr = (type *)get_dc_end_ptr(src_dc); \
	int _qy1 = py; \
	int _qy2 = py; \
	while (_qy1 < ph) { \
		if (_src_ptr >= _src_yend_ptr) break; \
		template_strecth_line_1(type, _dst_ptr, sx1, _src_ptr, sx2, \
			_src_xend_ptr, px, pw, op, arg); \
		_qy1 += sy1; \
		_dst_ptr = next_line(dst_dc, _dst_ptr); \
		if (_qy1 > _qy2) { \
 ...
 * : sy1 < sy2, sx1 >= sx2
 */
#define template_strecth_rect_2(type, dst_dc, x1, y1, sx1, sy1, \
	src_dc, x2, y2, sx2, sy2, \
	px, py, pw, ph, op, arg) \
{ \
	type *_dst_ptr = (type *)get_dc_wr_ptr(dst_dc, x1, y1); \
	...
	while (_qy1 < ph) { \
		if (_src_ptr >= _src_yend_ptr) break; \
		template_strecth_line_2(type, _dst_ptr, sx1, _src_ptr, sx2, \
			_src_xend_ptr, px, pw, op, arg); \
		_qy1 += sy1; \
		_dst_ptr = next_line(dst_dc, _dst_ptr); \
		if (_qy1 > _qy2) { \
 ...
 * : sy1 >= sy2, sx1 < sx2
 */
#define template_strecth_rect_3(type, dst_dc, x1, y1, sx1, sy1, \
	src_dc, x2, y2, sx2, sy2, \
	px, py, pw, ph, op, arg) \
{ \
	type *_dst_ptr = (type *)get_dc_wr_ptr(dst_dc, x1, y1); \
 ...
 * : sy1 >= sy2, sx1 >= sx2
 */
#define template_strecth_rect_4(type, dst_dc, x1, y1, sx1, sy1, \
	src_dc, x2, y2, sx2, sy2, \
	px, py, pw, ph, op, arg) \
{ \
	type *_dst_ptr = (type *)get_dc_wr_ptr(dst_dc, x1, y1); \
...
#define template_stretch_blt(type, dst_dc, x1, y1, sx1, sy1, \
	src_dc, x2, y2, sx2, sy2, \
	px, py, pw, ph, op, arg) \
{ \
	if ((sy1) < (sy2)) { \
		if ((sx1) < (sx2)) { \
			template_strecth_rect_1(type, dst_dc, (x1), (y1), (sx1), (sy1), \
				src_dc, (x2), (y2), (sx2), (sy2), \
				(px), (py), (pw), (ph), op, arg); \
		} else { \
			template_strecth_rect_2(type, dst_dc, (x1), (y1), (sx1), (sy1), \
				src_dc, (x2), (y2), (sx2), (sy2), \
				(px), (py), (pw), (ph), op, arg); \
		} \
	} \
		...
		if ((sx1) < (sx2)) { \
			template_strecth_rect_3(type, dst_dc, (x1), (y1), (sx1), (sy1), \
				src_dc, (x2), (y2), (sx2), (sy2), \
				(px), (py), (pw), (ph), op, arg); \
		} else { \
			template_strecth_rect_4(type, dst_dc, (x1), (y1), (sx1), (sy1), \
				src_dc, (x2), (y2), (sx2), (sy2), \
				(px), (py), (pw), (ph), op, arg); \
		} \
	} \
...
{ \
	...
	/* Region 2 */ \
	_p = _b2 * (_x + 0.5) * (_x + 0.5) + _a2 * (_y - 1) * (_y - 1) - _a2 * _b2; \
	while (_y > 0) { \
		--_y; \
		_py -= _two_a2; \
		if (_p > 0) { \
...
{ \
	...
	if (_y3 > _y0) { \
		if (_x3 - _x0 > _y3 - _y0) { \
			template_bres_draw_line_1(type, dc, _x0, _y0, _x3, _y3, color); \
		} else { \
			template_bres_draw_line_2(type, dc, _x0, _y0, _x3, _y3, color); \
		} \
	} else { \
		if (_x3 - _x0 > _y0 - _y3) { \
			template_bres_draw_line_3(type, dc, _x0, _y0, _x3, _y3, color); \
		} else { \
			template_bres_draw_line_4(type, dc, _x0, _y0, _x3, _y3, color); \
		} \
	} \
}
...
#define template_draw_char(type, dc, ch, x, y, left, top, right, bottom, \
	bg_color, txt_color) \
{ \
