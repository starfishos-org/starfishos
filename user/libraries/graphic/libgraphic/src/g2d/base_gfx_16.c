...
/* For transparent_blt */
static inline void transparent_pixel_16(RGB565 *dst, const RGB565 *src, RGB565 color)
{
...
static void draw_arc_16(const PDC dc, int x1, int y1, int x2, int y2,
...
static void draw_char_16(const PDC dc, char ch, int x, int y,
	int l, int t, int r, int b)
{
	RGB565 bg_color = rgb565_from_rgb(dc_bg_color(dc).rgb);
	RGB565 txt_color = rgb565_from_rgb(dc_txt_color(dc).rgb);
	template_draw_char(RGB565, dc, ch, x, y, l, t, r, b,
		bg_color, txt_color);
}

static void bitblt_16(const PDC dst_dc, int x1, int y1,
	...
	int w, int h, enum BLT_OP bop, void *arg)
{
	...
	case BLT_BLEND:
		alpha = *(u8 *)arg;
		template_bitblt(RGB565, dst_dc, x1, y1, src_dc, x2, y2, w, h,
			blend_pixel_16, alpha);
		break;
	case BLT_TRANSPARENT:
		color = rgb565_from_rgb((*(RGBA *)arg).rgb);
		template_bitblt(RGB565, dst_dc, x1, y1, src_dc, x2, y2, w, h,
			transparent_pixel_16, color);
		break;
	default:
		break;
	}
	...
	int px, int py, int pw, int ph, enum BLT_OP bop, void *arg)
{
	...
	case BLT_RASTER:
		...
		{
		case ROP_SRCAND:
			template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcand_16, NULL);
			break;
		case ROP_SRCCOPY:
			template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srccopy_16, NULL);
			break;
		case ROP_SRCERASE:
			template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcerase_16, NULL);
			break;
		case ROP_SRCINVERT:
			template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcinvert_16, NULL);
			break;
		case ROP_SRCPAINT:
			template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcpaint_16, NULL);
			break;
		default:
			break;
		}
	...
	case BLT_BLEND:
		...
		template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
			src_dc, x2, y2, sx2, sy2,
			px, py, pw, ph, blend_pixel_16, alpha);
		break;
	case BLT_TRANSPARENT:
		color = rgb565_from_rgb((*(RGBA *)arg).rgb);
		template_stretch_blt(RGB565, dst_dc, x1, y1, sx1, sy1,
			src_dc, x2, y2, sx2, sy2,
			px, py, pw, ph, transparent_pixel_16, color);
		break;
	default:
		break;
	}
...
static struct base_gfx base_gfx_16 = {
...
inline struct base_gfx *get_base_gfx_16()
{
	return &base_gfx_16;
}
