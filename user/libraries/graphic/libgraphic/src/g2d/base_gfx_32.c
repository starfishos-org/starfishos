...
static void draw_arc_32(const PDC dc, int x1, int y1, int x2, int y2,
...
static void draw_char_32(const PDC dc, char ch, int x, int y,
	int l, int t, int r, int b)
{
	RGBA bg_color = dc_bg_color(dc);
	RGBA txt_color = dc_txt_color(dc);
	template_draw_char(RGBA, dc, ch, x, y, l, t, r, b,
		bg_color, txt_color);
}

static void bitblt_32(const PDC dst_dc, int x1, int y1,
	...
	int w, int h, enum BLT_OP bop, void *arg)
{
	...
	case BLT_BLEND:
		alpha = *(u8 *)arg;
		template_bitblt(RGBA, dst_dc, x1, y1, src_dc, x2, y2, w, h,
			blend_pixel_32, alpha);
		break;
	case BLT_BLEND_RGBA:
		alpha = *(u8 *)arg;
		template_bitblt(RGBA, dst_dc, x1, y1, src_dc, x2, y2, w, h,
			blend_pixel_rgba, alpha);
		break;
	case BLT_TRANSPARENT:
		color = *(RGBA *)arg;
		template_bitblt(RGBA, dst_dc, x1, y1, src_dc, x2, y2, w, h,
			transparent_pixel_32, color);
		break;
	default:
		break;
	}
	...
	int px, int py, int pw, int ph, enum BLT_OP bop, void *arg)
{
	...
	case BLT_RASTER:
		...
		{
		case ROP_SRCAND:
			template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcand_32, NULL);
			break;
		case ROP_SRCCOPY:
			template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srccopy_32, NULL);
			break;
		case ROP_SRCERASE:
			template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcerase_32, NULL);
			break;
		case ROP_SRCINVERT:
			template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcinvert_32, NULL);
			break;
		case ROP_SRCPAINT:
			template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
				src_dc, x2, y2, sx2, sy2,
				px, py, pw, ph, rop_srcpaint_32, NULL);
			break;
		default:
			break;
		}
	...
	case BLT_BLEND:
		...
		template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
			src_dc, x2, y2, sx2, sy2,
			px, py, pw, ph, blend_pixel_32, alpha);
		break;
	case BLT_BLEND_RGBA:
		alpha = *(u8 *)arg;
		template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
			src_dc, x2, y2, sx2, sy2,
			px, py, pw, ph, blend_pixel_rgba, alpha);
		break;
	case BLT_TRANSPARENT:
		color = *(RGBA *)arg;
		template_stretch_blt(RGBA, dst_dc, x1, y1, sx1, sy1,
			src_dc, x2, y2, sx2, sy2,
			px, py, pw, ph, transparent_pixel_32, color);
		break;
	default:
		break;
	}
...
static struct base_gfx base_gfx_32 = {
...
inline struct base_gfx *get_base_gfx_32()
{
	return &base_gfx_32;
}
