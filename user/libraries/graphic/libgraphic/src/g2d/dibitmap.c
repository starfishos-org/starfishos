...
struct dib_header
{
	...
	u32 colors_used;		/* Number of color indexes in the color table that are actually used by the DIB */
	u32 colors_required;		/* Number of color indexes that are required for displaying the DIB */
};
...
static int dib_read_header(PDIB dib, int fd)
{
	 ...
	 */
	if (!read_u16(&(dib->header.magic), fd))		return DIB_IO_ERR;
	if (!read_u32(&(dib->header.file_size), fd))		return DIB_IO_ERR;
	if (!read_u16(&(dib->header.reserved1), fd))		return DIB_IO_ERR;
	if (!read_u16(&(dib->header.reserved2), fd))		return DIB_IO_ERR;
	if (!read_u32(&(dib->header.data_offset), fd))		return DIB_IO_ERR;
	if (!read_u32(&(dib->header.header_size), fd))		return DIB_IO_ERR;
	if (!read_u32(&(dib->header.w), fd))			return DIB_IO_ERR;
	if (!read_u32(&(dib->header.h), fd))			return DIB_IO_ERR;
	if (!read_u16(&(dib->header.planes), fd))		return DIB_IO_ERR;
	if (!read_u16(&(dib->header.bpp), fd))			return DIB_IO_ERR;
	if (!read_u32(&(dib->header.compression_type), fd))	return DIB_IO_ERR;
	if (!read_u32(&(dib->header.img_data_size), fd))	return DIB_IO_ERR;
	if (!read_u32(&(dib->header.hpixels_per_meter), fd))	return DIB_IO_ERR;
	if (!read_u32(&(dib->header.vpixels_per_meter), fd))	return DIB_IO_ERR;
	if (!read_u32(&(dib->header.colors_used), fd))		return DIB_IO_ERR;
	if (!read_u32(&(dib->header.colors_required), fd))	return DIB_IO_ERR;
...
static int dib_write_header(const PDIB dib, int fd)
{
	 ...
	 */
	if (!write_u16(dib->header.magic, fd))			return DIB_IO_ERR;
	if (!write_u32(dib->header.file_size, fd))		return DIB_IO_ERR;
	if (!write_u16(dib->header.reserved1, fd))		return DIB_IO_ERR;
	if (!write_u16(dib->header.reserved2, fd))		return DIB_IO_ERR;
	if (!write_u32(dib->header.data_offset, fd))		return DIB_IO_ERR;
	if (!write_u32(dib->header.header_size, fd))		return DIB_IO_ERR;
	if (!write_u32(dib->header.w, fd))			return DIB_IO_ERR;
	if (!write_u32(dib->header.h, fd))			return DIB_IO_ERR;
	if (!write_u16(dib->header.planes, fd))			return DIB_IO_ERR;
	if (!write_u16(dib->header.bpp, fd))			return DIB_IO_ERR;
	if (!write_u32(dib->header.compression_type, fd))	return DIB_IO_ERR;
	if (!write_u32(dib->header.img_data_size, fd))		return DIB_IO_ERR;
	if (!write_u32(dib->header.hpixels_per_meter, fd))	return DIB_IO_ERR;
	if (!write_u32(dib->header.vpixels_per_meter, fd))	return DIB_IO_ERR;
	if (!write_u32(dib->header.colors_used, fd))		return DIB_IO_ERR;
	if (!write_u32(dib->header.colors_required, fd))	return DIB_IO_ERR;
...
PDIB _load_dib(const char *filename)
{
	...
	if (dib->header.img_data_size == 0) {
		dib->header.img_data_size = dib->header.w * dib->header.h
			* (dib->header.bpp >> 3);
	}
	...
	if (palette_size > 0) {
		...
		if (read(fd, dib->palette, palette_size * sizeof(u8)) != palette_size) {
			dib_last_err_code = DIB_FILE_INVALID;
			close(fd);
			free(dib->palette);
			free(dib);
...
void _dib_get_pixel_rgb(const PDIB dib, u32 x, u32 y, u8 *r, u8 *g, u8 *b)
{
	...
	if (dib == NULL || x < 0 || x >= dib->header.w
		|| y < 0 || y >= dib->header.h)
		dib_last_err_code = DIB_INVALID_ARG;
	else {
		dib_last_err_code = DIB_OK;
		...
		pixel = dib->data + ((dib->header.h - y - 1) * bytes_per_row
					+ x * bytes_per_pixel);
		/*
		 ...
		 */
		if (dib->palette != NULL) {
			pixel = dib->palette + *pixel * 4; // TODO - por para 4bpp
		}
...
void _dib_get_pixel_index(const PDIB dib, u32 x, u32 y, u8 *val)
{
	...
	if (dib == NULL || val == NULL || x < 0 || x >= dib->header.w
		|| y < 0 || y >= dib->header.h)
		dib_last_err_code = DIB_INVALID_ARG;
	else if (dib->palette == NULL)
		dib_last_err_code = DIB_TYPE_MISMATCH;
		...
		if (dib->header.bpp == 8) {
			pixel = dib->data + ((dib->header.h - y - 1)
				* bytes_per_row + x);
			*val = *pixel;
		}
...
void _dib_get_palette_color(const PDIB dib, u8 index, u8 *r, u8 *g, u8 *b)
{
	if (dib == NULL || (dib->header.bpp == 4 && index >= 16))
		...
		/* Note: colors are stored in BGR order */
		if (r) *r = *(dib->palette + (index << 2) + 2);
		if (g) *g = *(dib->palette + (index << 2) + 1);
		if (b) *b = *(dib->palette + (index << 2) + 0);
		dib_last_err_code = DIB_OK;
	}
}
 ...
 */
inline enum DIB_STATUS _dib_get_error()
{
 ...
 */
inline const char* _dib_get_error_desc()
{
