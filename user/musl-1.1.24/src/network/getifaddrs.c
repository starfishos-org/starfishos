...
int getifaddrs(struct ifaddrs **ifap)
{
	...
	return r;
#elif 1 /* Temporary Implementation for multicast and ros2 demos */

	WARN("TODO: add the support of getifadrs in LWIP\n");
	...
	ifa_next = malloc(sizeof(struct ifaddrs));

	/* Configure ifa */
	memset(ifa, 0, sizeof(struct ifaddrs));
	ifa->ifa_next = ifa_next;
	ifa->ifa_name = malloc(sizeof(16));
	strcpy(ifa->ifa_name, "LOCALHOST");
	...
	/* Configure ifa_next */
	memset(ifa_next, 0, sizeof(struct ifaddrs));
	ifa_next->ifa_next = NULL;
	ifa_next->ifa_name = malloc(sizeof(4));
	strcpy(ifa_next->ifa_name, "lo");
	ifa_next->ifa_flags = IFF_UP | IFF_LOOPBACK | IFF_RUNNING;
	...
	ifa_next->ifa_addr = (struct sockaddr *)addr;
	mask->sin_family = AF_INET;
	mask->sin_port = 0;
	mask->sin_addr.s_addr = inet_addr("255.255.255.0");
	ifa_next->ifa_netmask = (struct sockaddr *)mask;

	*ifap = ifa;

	return 0;
#endif
	BUG_ON("getifaddrs not implemented.");
	return -1;
}
