...
int launch_process_with_pmos_caps(struct launch_process_args *lp_args)
{
	...
	for (i = 0; i < ELF_MAX_LOAD_SEG; ++i) {
		...
		pmo_map_requests[2 + i].pmo_cap = user_elf->user_elf_seg[i].elf_pmo;
		pmo_map_requests[2 + i].addr = ROUND_DOWN(user_elf->user_elf_seg[i].p_vaddr, PAGE_SIZE);
