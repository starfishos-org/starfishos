...
void __libc_connect_services(char *envp[])
{
	...
	procmgr_server_cap = (int)(caps[0]);

	/* Connect to FS only */
	if ((--nr_caps) == 0) return;

	fsm_server_cap = (int)(caps[1]);

	/* Connect to FS and NET */
	if ((--nr_caps) == 0) return;

	lwip_server_cap = (int)(caps[2]);
...
void __libc_chcore_init(char *envp[])
{
	...
	// BUG: this fd0 is a file descriptor, but fd_dic[fd0]->fd should be an "fid" in fsm.srv
	fd_dic[fd0]->fd = fd0;
...
long __syscall2(long n, long a, long b)
{
	...
	case SYS_fstat: {
		...
		return __xstatxx(FS_REQ_FSTAT, a, /* fd */
				 NULL,			      /* path  */
				 0,			      /* flags */
	...
	case SYS_fstatfs: {
		...
		return __xstatxx(FS_REQ_FSTATFS, a, /* fd */
				 NULL,				/* path  */
				 0,				/* flags */
				 (struct statfs *)b,		/* statbuf */
				 sizeof(struct statfs)		/* bufsize */
...
pid_t chcore_waitpid(pid_t pid, int *status, int options, int d)
{
	...
	if (ret > 0 && status != NULL) {
		/* Get the actual exit status; waitpid(pid, NULL, 0) is legal. */
...
long __syscall5(long n, long a, long b, long c, long d, long e)
{
	...
	case SYS_mount: {
		return chcore_mount((const char *)a, (const char *)b, (const char *)c,
...
long __syscall6(long n, long a, long b, long c, long d, long e, long f)
{
	...
	case SYS_nanosleep:
		 ...
		 */
		return chcore_syscall4(CHCORE_SYS_clock_nanosleep, CLOCK_MONOTONIC, 0, a, b);
	case SYS_getrandom: {
