...
static struct timespec ns_to_timespec(long ns)
{
	struct timespec time;
	time.tv_sec = ns / NS_IN_S;
	time.tv_nsec = ns % NS_IN_S;
	return time;
}
...
{
	long cur_ns, val_ns, interval_ns, remain_ns;
	struct timespec cur_time;
	struct timer_file *timer_file;
...
static int chcore_timerfd_read(int fd, void *buf, size_t count)
{
	long cur_ns, val_ns, interval_ns, next_expire_ns;
...
static int chcore_timerfd_write(int fd, void *buf, size_t count)
{
	return -EINVAL;
}
			      ...
			      int notifc_cap)
{
	...
	struct timer_file *timer_file;
	struct timespec cur_time;
	long wait_ns = 0x7FFFFFFFFFFFFFFF;
	long val_ns = 0, cur_ns = 0;
	...
	for (i = 0; i < nfds; i++) {
		...
		if (cur_ns > val_ns) { /* ready */
			fds[i].revents =
			    fds[i].events & POLLIN || fds[i].events & POLLRDNORM
				? POLLIN | POLLRDNORM
				: 0;
			count += 1;
		} else { /* not ready */
	...
	if (timeout > 0 && wait_ns > (long)timeout * 1000000)
		return 0;

	tpa = malloc(sizeof(struct timerfd_poll_arg));
	tpa->notifc_cap = notifc_cap;
	tpa->wait_ns = wait_ns;
	ret = pthread_create(&tid, NULL, timerfd_poll_thread_routine,
			     (void *)tpa);
	return ret;
}
