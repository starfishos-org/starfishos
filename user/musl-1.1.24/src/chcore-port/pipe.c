 ...
 */
void pipe_server_handler(ipc_msg_t *ipc_msg, u64 client_badge)
{
	...
	case PIPE_RQ_READ:
		ret = chcore_pipe_read_local(pipe_rq->fd, pipe_rq->data,
					     pipe_rq->count);
		break;
	case PIPE_RQ_WRITE:
		ret = chcore_pipe_write_local(pipe_rq->fd, pipe_rq->data,
					      pipe_rq->count);
		break;
	default:
		ret = -EINVAL;
...
volatile int pipe_server_thread_cap = 0;
...
int chcore_pipe2(int pipefd[2], int flags)
{
	...
	/*
	 * XXX: Init a thread to register pipe server. Though the thread is not used
	 * after init, a thread can only register one ipc handler. So init the
	 * thread to register pipe's handler.
	 */
...
static int chcore_pipe_read(int fd, void *buf, size_t count)
{
	ipc_msg_t *ipc_msg;
	struct pipe_request *pipe_rq;
	int ret;
	...
	ipc_msg = ipc_create_msg(pipe_ipc_struct,
				 sizeof(struct pipe_request) + count, 0);
...
static int chcore_pipe_write(int fd, void *buf, size_t count)
{
	ipc_msg_t *ipc_msg;
	struct pipe_request *pipe_rq;
	int ret;
