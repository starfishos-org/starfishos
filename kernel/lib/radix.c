...
static unsigned long __radix_checksum(unsigned long initial, struct radix_node *node, int level)
{
	int i;
	...
	if (level == RADIX_LEVELS - 1) {
		for (i = 0; i < RADIX_NODE_SIZE; i++) {
			if (!node->values[i])
				continue;
			initial = hash_page(initial, (unsigned char *)phys_to_virt(node->values[i]), PAGE_SIZE);
		}
		return initial;
	}
	for (i = 0; i < RADIX_NODE_SIZE; i++) {
...
static int __radix_deep_copy(struct radix_node *src, struct radix_node *dst, int node_level,int phy_alloc)
{
	int err;
	int i;
	struct radix_node *new;
...
static void __radix_traverse(struct radix_node *node, u64 prefix, int level, radix_traverse_fn fn)
{
	int i;
...
static void *__radix_complex_traverse(struct radix_node *node, u64 prefix, int level, 
void *args, radix_complex_traverse_fn fn)
{
	int i;
	prefix <<= RADIX_NODE_BITS;
	vaddr_t va;
	...
	if (level == 1) {
		...
		for (i = 0; i < RADIX_NODE_SIZE; i++) {
			if (node->children[i]) {
				return __radix_complex_traverse(node->children[i], prefix | i, level - 1, args, fn);
			}
		}
	}
	return NULL;
...
void *radix_complex_traverse(struct radix *radix, void * args, radix_complex_traverse_fn fn)
{
	void *res = NULL;
