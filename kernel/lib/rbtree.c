 ...
 * external nodes.
 */
static void __rb_insert_color(struct rb_root *root, struct rb_node *node)
{
    struct rb_node *parent = rb_parent(node), *gparent = NULL, *uncle = NULL;
        ...
        if (parent == gparent->left_child) {
            ...
            if (!uncle || rb_is_black(uncle)) {
                ...
                if (node == parent->left_child) {
                    ...
                    __rb_change_child(
                            root, rb_parent(gparent), gparent, parent);
                     ...
                     *       x                    U
                     */
                    __rb_change_child(root, rb_parent(gparent), gparent, node);
...
static struct rb_node *__rb_erase(struct rb_root *this, struct rb_node *node,
                                  bool *is_left_child)
{
    struct rb_node *succ, *parent = rb_parent(node), *ret = NULL, *succ_child;
        ...
        if (!succ->left_child) {
            ...
            if (rb_is_black(succ)) {
                if (succ_child && rb_is_red(succ_child)) {
                    ...
                    ret = parent;
                    *is_left_child = succ == parent->left_child;
                }
            }
    ...
    while (true) {
        ...
        if (node == parent->left_child) {
            ...
            if (rb_is_black(slibing)) {
                if (slibing->right_child && rb_is_red(slibing->right_child)) {
                    ...
                    __rb_change_child(root, rb_parent(parent), parent, slibing);
                     ...
                     *       n  (s1)      X      (s1)
                     */
                    __rb_change_child(root, rb_parent(parent), parent, nephew);
                    ...
                    if (rb_is_black(parent)) {
                         ...
                         *        / \               / \
                         *       S1  S2            S1  S2
                         * Subtree P has 1 lower blackheight than slibings
                         * of P. (X has 1 lower blackheight than S before).
                         * So we let @node = P and continue iteration.
                         */
                 ...
                 *        / \          / \
                 *       S1  S2       X   S1
                 * We do not fix rbtree here. However, after this
                 * rotation, X has a black slibing now, and will turn
                 * into Case-1 or Case-3 (depends on children of S1).
                 */
                __rb_change_child(root, rb_parent(parent), parent, slibing);

                parent->right_child = slibing->left_child;
                if (slibing->left_child) {
                    rb_set_parent(slibing->left_child, parent);
                }
            ...
            if (rb_is_black(slibing)) {
                if (slibing->left_child && rb_is_red(slibing->left_child)) {
                    // mirror of Case-1-A
                    nephew = slibing->left_child;
                ...
                if (slibing->right_child) {
                    rb_set_parent(slibing->right_child, parent);
                }
...
struct rb_node *rb_search_fisrt(struct rb_root *this, const void *key,
                                comp_key_func cmp)
{
