...
struct queue_meta {
	...
	char pad[pad_to_cache_line(sizeof(u32) + sizeof(struct list_head)+sizeof(struct lock))];
};
        ...
        for (j = 0; j < TEST_NUM; j++) {
				...
				if (is_valid_prev_thread(current_thread->prev_thread))
					current_thread->prev_thread->thread_ctx->kernel_stack_state = KS_FREE;
                        ...
                        /*
                         * It should be mentioned that sched() will go through the FPU save/restore
                         * procedure which might modify the value of sstatus register and result in
                         * the failure of this test. Fortunately, in this test the sstatus register
                         * will remain the same.
                         * But future development must bear this in mind.
                         */
                        for (k = 0; k < REG_NUM; k++)
                                BUG_ON(thread_ctx->ec.reg[k] != thread_ctx->prio);
                }
