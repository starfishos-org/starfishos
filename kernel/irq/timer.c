...
#ifdef DSM_ENABLED
        /* Skip budget decrement if thread is migrating to remote or already
         * migrated */
        if (current_thread->thread_ctx->thread_exit_state == TE_MIGRATING
            || current_thread->machine_id != CUR_MACHINE_ID) {
            /* Thread is being migrated or already on remote machine, skip
             * budget decrement */
        } else
#endif
        {
...
/*
 * clock_gettime:
 * - TODO: now we simply ignore the clockid (treat all as monotime)
