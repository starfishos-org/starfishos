                       ...
                       u64 stack, u64 pc, u32 prio, u32 type, s32 aff)
{
    ...
    thread->cap_group = obj_get(cap_group, CAP_GROUP_OBJ_ID, TYPE_CAP_GROUP);
...
/* Required by LibC */
void prepare_env(char *env, u64 top_vaddr, char *name,
...
void create_root_thread(void)
{
    ...
    kva = (vaddr_t)get_pages(0, __MT_STACK__);
    BUG_ON(kva == 0);
...
void thread_clone(struct cap_group *cap_group, struct thread *thread)
{
    ...
    ret = thread_init(thread,
                      ...
                      0,
                      0,
                      current_thread->thread_ctx->prio,
                      TYPE_USER,
                      NO_AFF);
    if (ret != 0)
    ...
    memcpy(&thread->thread_ctx->ec,
           &current_thread->thread_ctx->ec,
           ARCH_EXEC_CONT_SIZE);
    memcpy(&thread->thread_ctx->tls_base_reg,
           &current_thread->thread_ctx->tls_base_reg,
...
void sys_thread_exit(void)
{
    ...
    printk("<track> thread of %s: kernel %d ms, user %d ms\n",
           ...
           current_thread->track_time_kernel,
           current_thread->track_time_user);
#endif
