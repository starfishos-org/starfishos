...
int alloc_slot_id(struct cap_group *cap_group)
{
    ...
    while (true) {
        ...
        empty_idx = find_next_zero_bit(
                slot_table->slots_bmp, bmp_size, empty_idx * BITS_PER_LONG);
    ...
    if (slot_table->slots_bmp[empty_idx / BITS_PER_LONG] == ~((unsigned long)0))
...
int sys_create_cap_group(u64 badge, u64 cap_group_name, u64 name_len, u64 pcid)
{
    ...
    copy_from_user(
            new_cap_group->cap_group_name, (char *)cap_group_name, name_len);
    return cap;
...
int sys_clone_cap_group(u64 clone_cap_group_args)
{
    ...
    args.lwip_server_cap =
            cap_copy(current_cap_group, new_cap_group, args.lwip_server_cap);
    args.procmgr_server_cap =
            cap_copy(current_cap_group, new_cap_group, args.procmgr_server_cap);
    args.child_mt_cap = r;
