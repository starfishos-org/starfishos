...
int sys_create_cap_group(u64 badge, u64 cap_group_name, u64 name_len, u64 pcid)
{
	...
	// if(new_cap_group->cap_group_name[1] == 'r' && new_cap_group->cap_group_name[7] == 'b') {
	if(new_cap_group->cap_group_name[1] == 'r' && new_cap_group->cap_group_name[7] == 's') {
		extern struct vmspace* redis_vmspace;
		redis_vmspace = vmspace;
	}
...
int sys_clone_cap_group(u64 clone_cap_group_args)
{
	...
	r = copy_from_user((char *)&args, (char *)clone_cap_group_args, sizeof(args));
	BUG_ON(r);
	...
	memcpy(new_cap_group->cap_group_name, current_cap_group->cap_group_name,
	       MAX_GROUP_NAME_LEN);
	 ...
	 */
	args.fs_server_cap = cap_copy(current_cap_group, new_cap_group, args.fs_server_cap);
	args.lwip_server_cap = cap_copy(current_cap_group, new_cap_group, args.lwip_server_cap);
	args.procmgr_server_cap = cap_copy(current_cap_group, new_cap_group, args.procmgr_server_cap);
	args.child_mt_cap = r;
	args.parent_badge = current_cap_group->badge;
	r = copy_to_user((char *)clone_cap_group_args, (char *)&args, sizeof(args));
	BUG_ON(r);

	/* 5. Clone the original vmspace */
	r = vmspace_clone(child_vmspace, current_thread->vmspace, new_cap_group);
	if (r < 0) {
		/* Deinitialize the child vmspace if error happens in vmspace_clone(). */
