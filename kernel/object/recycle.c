...
void sys_exit_group(int exitcode)
{
	...
	if (__sync_val_compare_and_swap(&current_cap_group->notify_recycler,
		...
		for_each_in_list(thread, struct thread, node,
			...
			/* CAS is used in case the state is set to TE_EXITED concurrently */
			__sync_val_compare_and_swap(
					&thread->thread_ctx->thread_exit_state,
					TE_RUNNING, TE_EXITING);
#ifdef TRACK_TIME
			printk("<track> thread: kernel %d ms, user %d ms\n", current_thread->track_time_kernel / 1000000, current_thread->track_time_user / 1000000);
#endif
		}
					 ...
					 struct thread *server_thread)
{
	...
	if (config->ipc_exit_routine_entry) {
		...
		/*
			...
			*/
		server_thread->thread_ctx->sc = kmalloc(
			sizeof(*server_thread->thread_ctx->sc));
		...
		vmspace_unmap_range(
			conn->server_handler_thread->vmspace,
...
/* Wait onging IPCs to finish and stop new IPCs. Also, recycle the connection cap. */
...
static void stop_ipc_registration(struct cap_group *cap_group, struct object_slot *slot, int *ret)
{
	struct thread *thread;
...
/*
 * FIXME:
 * Suppose cap_group is exiting, what if a thread in another cap_group transfer cap to it.
 */
...
int sys_cap_group_recycle(int cap_group_cap)
{
	...
	for_each_set_bit(slot_id, slot_table->slots_bmp,
		...
		if ((object->type == TYPE_THREAD) &&
			...
			/* Not using obj_get or get_opaque is also for avoid deadlock. */
			...
			for_each_in_list_safe(slot_iter, slot_iter_tmp,
				...
				// r = 
				__cap_free(iter_cap_group, iter_slot_id,
					       true, true);
				// BUG_ON(r != 0);
			}
...
int recycle_create_ckpt(struct ckpt_recycle_data* recycle_data) 
{
	...
	struct recycle_msg_node ** recycle_msg_array = kmalloc(msg_list_size * sizeof(struct recycle_msg_node *));	
	struct recycle_msg_node * msg;
	int i = 0;
	for_each_in_list(msg, struct recycle_msg_node, node,
				 &recycle_msg_head) {
...
int recycle_restore(struct ckpt_recycle_data* recycle_data, struct kvs* obj_map) 
{
	...
	recycle_msg_buffer = restore_ring_buffer(recycle_data->recycle_msg_buffer);
	
	/* restore recycle_msg_array */
	init_list_head(&recycle_msg_head);
	msg_list_size = recycle_data->recycle_msg_size;
	lock_init(&recycle_buffer_lock);
	struct recycle_msg_node ** recycle_msg_array = recycle_data->recycle_msg_array;
	struct recycle_msg_node * msg;
	...
	recycle_notification = (struct notification *)restore_obj_get(
					recycle_data->recycle_notification_obj_root)->opaque;
	BUG_ON(!recycle_notification);
	return 0;
}
