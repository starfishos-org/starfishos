                ...
                if (src_pmo->dram_cache.array != NULL) {
                        ...
                        *is_cow = false;
                        ...
                        for (i = 0; i < page_num; i++) {
                                u64 src_pa,
                                        dst_pa = dst_pmo->start + i * PAGE_SIZE;
                                if (array[i] != 0) {
                                        src_pa = array[i];
                                } else {
                        ...
                        page = virt_to_page(
                                (void *)phys_to_virt(src_pmo->start));
...
{
        u64 type = pmo->type;
        if (type == PMO_ANONYM || type == PMO_FILE || type == PMO_SHM
...
{
        if (pmo->type == PMO_RING_BUFFER || pmo->type == PMO_RING_BUFFER_RADIX
            || pmo->type == PMO_DEVICE)
                return true;
        return false;
}
