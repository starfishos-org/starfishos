	...
	if (pmo_type == PMO_DATA || pmo_type == PMO_DATA_NOCACHE) {
		...
		while (size > 0) {
			...
			* the offset inside the page. */
			...
			if (op_type == WRITE)
				r = copy_from_user((char *)kva, (char *)user_buf, to_read_write);
			else // op_type == READ
 ...
 */
int sys_map_pmo(u64 target_cap_group_cap, u64 pmo_cap, u64 addr, u64 perm, u64 len)
{
...
int sys_map_pmos(u64 target_cap_group_cap, u64 user_buf, u64 cnt)
{
	...
	for (i = 0; i < cnt; ++i) {
		...
		if (ret >= 0 && requests[i].free_cap == 1) {
			pmo = obj_get(current_cap_group, requests[i].pmo_cap,
				      TYPE_PMO);
			vmspace = obj_get(current_cap_group, VMSPACE_OBJ_ID,
					  TYPE_VMSPACE);
 ...
 */
static int pmo_init(struct pmobject *pmo, pmo_type_t type, size_t len, paddr_t paddr)
{
	int ret = 0;
	...
	case PMO_FILE: {
		 ...
		 * We store PMO_FILE metadata in fmap_fault_pool in pmo->private,
		 * and pmo->private is initialized NULL by memset.
		 */
		struct fmap_fault_pool *pool_iter;
		u64 badge;
		badge = current_cap_group->badge;
		lock(&fmap_fault_pool_list_lock);
		for_each_in_list(pool_iter, struct fmap_fault_pool, node, &fmap_fault_pool_list) {
