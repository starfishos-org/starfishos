	       ...
	       bool slot_table_locked, bool copies_list_locked)
{
	...
	/* Step-2: remove the slot in the copies-list of the object and free the slot */
	object = slot->object;
...
int cap_copy(struct cap_group *src_cap_group, struct cap_group *dest_cap_group, int src_slot_id)
{
#ifdef CKPT_CAP_GROUP_LAZY_COPY
...
int cap_move(struct cap_group *src_cap_group, struct cap_group *dest_cap_group, int src_slot_id)
{
	int r;
