...
#define next_container_of_safe(obj, type, field)                         \
    ({                                                                   \
        ...
        (__obj ? container_of_safe(((__obj)->field).next, type, field) : \
                 NULL);                                                  \
    })
...
#define for_each_in_list_backward(elem, type, field, head) \
...
#define __for_each_in_list_safe_direct_start(elem, tmp, type, field, head) \
    for (elem = container_of((head), type, field),                         \
        tmp = next_container_of_safe(elem, type, field);                   \
         ;                                                                 \
         elem = tmp, tmp = next_container_of_safe(tmp, type, field))

#define for_each_in_list_safe_direct_start(elem, tmp, field, head) \
    __for_each_in_list_safe_direct_start(elem, tmp, typeof(*elem), field, head)
...
#define __for_each_in_hlist(elem, type, field, head)                          \
    for (elem = container_of_safe((head)->next, type, field); elem;           \
         elem = elem ? container_of_safe(((elem)->field).next, type, field) : \
                       NULL)
