...
void sched_to_thread(struct thread *target)
{
	...
	if ((is_fpu_owner >= 0) &&
		...
		/* Fast path: direct switch to target thread (without scheduling). */

		/*
		 * TODO: if disallow sched_to_thread in notification,
		 * we can add BUG_ON(current_thread->thread_ctx->state != TS_WAITING) here
		 * and remove the below if statement.
		 */
 ...
 * ** Shoule hold a dedicated lock for the thread_list and this function can only
 * be called in the critical section!! **
 * Only the thread which kernel state is free can be choosed
 */
...
static void init_idle_threads(void)
{
	...
	extern struct vmspace *create_idle_vmspace(void); // in arch/mm/vmspace.c
	...
	for (i = 0; i < PLAT_CPU_NUM; i++) {
		...
		arch_idle_ctx_init(
			idle_threads[i].thread_ctx, 
			(u64)idle_thread_stack + (i + 1) * DEFAULT_STACK_SIZE,
			idle_thread_routine);
