...
struct ckpt_ws_data *get_ckpt_ws_data()
{
	...
	if (likely(second_latest_ws_data)) {
		...
		if (latest_ws_data) {
			data->version_number =
					latest_ws_data->version_number + 1;
		} else {
...
struct ckpt_object *ckpt_obj_get(struct ckpt_obj_root *ckpt_obj_root, int alloc)
{
	...
	if (system_current_flip_flag == ckpt_obj_root->flip_flag) {
		...
		BUG_ON(!(ckpt_obj = get_second_latest_ckpt_obj(
	...
	if (likely(alloc)) {
		...
		if (!(ckpt_obj = get_second_latest_ckpt_obj(
						ckpt_obj_root, current_ckpt_verison))) {
		...
		case TYPE_CAP_GROUP: {
			r = cap_group_copy_ckpt(
					(struct cap_group *)obj->opaque,
		...
		case TYPE_THREAD: {
			...
			char *name = ((struct thread *)obj->opaque)->cap_group->cap_group_name;
			if (!strcmp(name, "/ycsbc")
				|| !strcmp(name, "/redis_benchmark")
		...
		case TYPE_CONNECTION: {
			r = connection_ckpt(
					(struct ipc_connection *)obj->opaque,
		...
		case TYPE_NOTIFICATION: {
			r = notification_ckpt(
					(struct notification *)obj->opaque,
		...
		case TYPE_IRQ: {
			r = irq_ckpt(
					(struct irq_notification *)obj->opaque,
