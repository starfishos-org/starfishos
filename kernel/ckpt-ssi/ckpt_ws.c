...
int ckpt_ws_init(void)
{
	...
	CKPT_WS_TABLE =
	    (struct ckpt_ws_table *)kmalloc(sizeof(struct ckpt_ws_table), __SHARED__);
	if (!CKPT_WS_TABLE) {
...
int __name_kvs_put(struct ckpt_ws_info *info)
{
	...
	if (!info_list_head_val) {
		info_list_head =
		    (ckpt_ws_info_list_t *)kmalloc(sizeof(*info_list_head), __SHARED__);
		if (!info_list_head) {
...
u64 ckpt_ws_put_from_userspace(struct ckpt_ws_data *ckpt_data, u64 name_buf, u64 name_len)
{
	...
	if (name_buf) {
		...
		if (name_len > MAX_CKPT_NAME_LEN) {
			kinfo(
			    "[CKPT WS] only support name len < %d, truncked\n",
			    MAX_CKPT_NAME_LEN);
			info->name_len = MAX_CKPT_NAME_LEN;
		} else
			info->name_len = name_len;

		ret = copy_from_user((char *)(info->name), (char *)name_buf,
				     info->name_len);
		if (ret) {
