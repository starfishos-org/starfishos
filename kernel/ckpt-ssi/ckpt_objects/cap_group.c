...
int slot_table_ckpt(struct cap_group *cap_group,struct ckpt_cap_group *ckpt_cap_group)
{
#ifdef REPORT
    DECLTMR; start();
#endif
	...
	for (i = 0; i < bmp_long_count; i++) {
		...
		for (offset = 0, map = slot_table->slots_bmp[i]; map; offset++, map >>= 1) {
			if (map & 1) {
				ckpt_table_size++;
			}
		}
    ...
    for (i = 0; i < bmp_long_count; i++) {
        ...
        for (; map; offset++, map >>= 1) {
			if (map & 1) {
				...
				new_obj_root = ckpt_obj_root_get(slot_table->slots[slot_id]->object,true);
				BUG_ON(!new_obj_root);
#ifdef REPORT
                stop();
#endif
...
int cap_group_copy_ckpt(struct cap_group *cap_group,struct ckpt_cap_group *ckpt_cap_group)
{
    ...
    if (ckpt_cap_group->cap_group_name[0] == 0)
        memcpy(ckpt_cap_group->cap_group_name, cap_group->cap_group_name, MAX_GROUP_NAME_LEN);
    
    /* Copy slot table */
    return slot_table_ckpt(cap_group,ckpt_cap_group);
}

int slot_table_restore(struct cap_group *cap_group,struct ckpt_cap_group *ckpt_cap_group,
				struct kvs *obj_map)
{
                      ...
                      bool time_traveling)
{
    ...
    /* cap_group init */
    cap_group_init(cap_group, ckpt_cap_group->slots_size, ckpt_cap_group->badge);
    memcpy(cap_group->cap_group_name, ckpt_cap_group->cap_group_name, MAX_GROUP_NAME_LEN);
