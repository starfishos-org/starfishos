 ...
 */
struct slab_pointer static_slab_pool[SLAB_MAX_ORDER + 1];
...
void init_slab(void)
{
    ...
    for (order = SLAB_MIN_ORDER; order <= SLAB_MAX_ORDER; order++) {
        ...
        /* If we need to store from NVM on disk,
        Omit init current_slab and partial_slab_list */
        if (NVM_IS_CRASH) {
            kdebug("[Restore] Restore from old slab.\n");
            continue;
        }
#endif
...
void *alloc_in_slab(unsigned long size)
{
    int order;
