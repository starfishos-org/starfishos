 ...
 * 1: node1 > node2
 */
static bool cmp_two_vmrs(const struct rb_node *node1, const struct rb_node *node2)
{
			       ...
			       struct vmregion *vmr_to_add)
{
	...
	struct rb_node *res;
	res = rb_search(&vmspace->vmr_tree, (const void *)&range, cmp_vmr_and_range);
	/*
...
static void remove_vmr_from_vmspace(struct vmspace *vmspace, struct vmregion *vmr)
{
	if (check_vmr_intersect(vmspace, vmr) != 0) {
...
void kprint_vmr(struct vmspace *vmspace)
{
	...
	rb_for_each(&vmspace->vmr_tree, node) {
		...
		kinfo("[%p] [vmregion] start=%p end=%p. vmr->pmo->type=%d\n",
								vmspace, start, end, vmr->pmo->type);
	}
}
...
static struct vmregion *find_vmr_by_pmo(struct vmspace *vmspace, struct pmobject *pmo)
{
	struct vmregion *vmr = NULL;
        struct vmregion *iter_vmr;

	/* Find the corresponding vmr of the given pmo */
	for_each_in_list(iter_vmr, struct vmregion, list_node, &vmspace->vmr_list) {
		if (iter_vmr->pmo == pmo) {
			vmr = iter_vmr;
			break;
		}
...
int pmo_copy(struct pmobject *src_pmo, struct pmobject *dst_pmo)
{
	...
	case PMO_RING_BUFFER_RADIX: {
		int phy_alloc = (src_pmo->type == PMO_SHM 
				|| src_pmo->type == PMO_RING_BUFFER_RADIX) ? 1 : 0;
