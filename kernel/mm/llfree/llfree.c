					    ...
					    uint64_t start)
{
	...
	reserve_args_t args = { .self = self, .order = order, .tier = tier, .local = local, .check_args = &check_args };
...
static llfree_result_t get_matching(llfree_t *self, const llfree_request_t *request, uint64_t *start)
{
	...
	if (request->local != LLFREE_LOCAL_NONE && tier_count != LLFREE_LOCAL_NONE && tier_count > 0 &&
		...
		for (size_t i = 0; i < 16; i++) {
			local_result_t old;
			llfree_result_t res =
				get_from_local(self, request->tier, request->local, request->order, frames, &old);
			if (old.present)
		...
		return get_matching_reserve(self, request->tier, request->local, request->order, *start);
	}
...
llfree_result_t llfree_get(llfree_t *self, ll_optional_t frame, llfree_request_t request)
{
	...
	if (frame.present) {
		...
		return lower_get(&self->lower, (uint64_t)frame.value, request.order, ll_some(frame.value));
	}
