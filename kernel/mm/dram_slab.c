...
/* Per-CPU DRAM slab pools/locks, indexed by [cpu][order]. */
struct slab_pointer dram_slab_pool[PLAT_CPU_NUM][SLAB_MAX_ORDER + 1];
static struct lock dram_slabs_locks[PLAT_CPU_NUM][SLAB_MAX_ORDER + 1];
...
static void *alloc_in_dram_slab_impl(int order)
{
    ...
    lock(&dram_slabs_locks[cpu_id][order]);

    current_slab = dram_slab_pool[cpu_id][order].current_slab;
    ...
    if (unlikely(current_slab == NULL)) {
        ...
        if (current_slab == NULL) {
            unlock(&dram_slabs_locks[cpu_id][order]);
            return NULL;
        }
        dram_slab_pool[cpu_id][order].current_slab = current_slab;
    }
    ...
    if (unlikely(current_slab->current_free_cnt == 0))
        choose_new_current_slab(&dram_slab_pool[cpu_id][order], order);
...
static void try_insert_full_slab_to_partial(struct slab_header *slab)
{
    ...
    list_append(&slab->node, &dram_slab_pool[owner_cpu][order].partial_slab_list);
}
...
static void try_return_slab_to_buddy(struct slab_header *slab, int order)
{
    ...
    if (slab == dram_slab_pool[owner_cpu][order].current_slab)
        choose_new_current_slab(&dram_slab_pool[owner_cpu][order], order);
    else
        list_del(&slab->node);
...
void init_dram_slab(void)
{
    ...
    for (cpu_id = 0; cpu_id < PLAT_CPU_NUM; cpu_id++) {
        for (order = SLAB_MIN_ORDER; order <= SLAB_MAX_ORDER; order++) {
            lock_init(&dram_slabs_locks[cpu_id][order]);
            dram_slab_pool[cpu_id][order].current_slab = NULL;
            init_list_head(&(dram_slab_pool[cpu_id][order].partial_slab_list));
        }
    }
...
void free_in_dram_slab(void *addr)
{
    ...
    owner_cpu = slab->owner_cpu;
    BUG_ON(owner_cpu >= PLAT_CPU_NUM);
    lock(&dram_slabs_locks[owner_cpu][order]);
    ...
    unlock(&dram_slabs_locks[owner_cpu][order]);
}
...
unsigned long dram_get_free_slot_number(int order)
{
    ...
    for (cpu_id = 0; cpu_id < PLAT_CPU_NUM; cpu_id++) {
        ...
        slab = dram_slab_pool[cpu_id][order].current_slab;
        if (slab) {
        ...
        if (!list_empty(&dram_slab_pool[cpu_id][order].partial_slab_list)) {
            for_each_in_list (slab,
                              struct slab_header,
                              node,
                              &dram_slab_pool[cpu_id][order].partial_slab_list) {
                slot = (struct slab_slot_list *)slab->free_list_head;
                while (slot != NULL) {
                    current_slot_num++;
                    slot = slot->next_free;
