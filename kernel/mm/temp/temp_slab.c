 ...
 */
struct slab_pointer temp_slab_pool[SLAB_MAX_ORDER + 1];
static struct lock temp_slabs_locks[SLAB_MAX_ORDER + 1];
...
static void *alloc_in_temp_slab_impl(int order)
{
    struct slab_header *current_slab;
    struct slab_slot_list *free_list;
    void *next_slot;

    lock(&temp_slabs_locks[order]);

    current_slab = temp_slab_pool[order].current_slab;
    ...
    if (unlikely(current_slab == NULL)) {
        ...
        if (current_slab == NULL) {
            unlock(&temp_slabs_locks[order]);
            return NULL;
        }
        temp_slab_pool[order].current_slab = current_slab;
    }
    ...
    if (unlikely(current_slab->current_free_cnt == 0))
        choose_new_current_slab(&temp_slab_pool[order], order);
...
static void try_insert_full_slab_to_partial(struct slab_header *slab)
{
    ...
    list_append(&slab->node, &temp_slab_pool[order].partial_slab_list);
}
...
static void try_return_slab_to_buddy(struct slab_header *slab, int order)
{
    ...
    if (slab == temp_slab_pool[order].current_slab)
        choose_new_current_slab(&temp_slab_pool[order], order);
    else
        list_del(&slab->node);
...
void init_temp_slab(void)
{
    ...
    for (order = SLAB_MIN_ORDER; order <= SLAB_MAX_ORDER; order++) {
        lock_init(&temp_slabs_locks[order]);
        temp_slab_pool[order].current_slab = NULL;
        init_list_head(&(temp_slab_pool[order].partial_slab_list));
    }
    kdebug("mm: finish initing slab allocators\n");
}

void *alloc_in_temp_slab(unsigned long size)
{
    int order;
    ...
    return alloc_in_temp_slab_impl(order);
}
 ...
 */
unsigned long temp_get_free_slot_number(int order)
{
    struct slab_header *slab;
    struct slab_slot_list *slot;
    ...
    slab = temp_slab_pool[order].current_slab;
    if (slab) {
    ...
    if (list_empty(&temp_slab_pool[order].partial_slab_list))
        goto out;

    for_each_in_list (slab,
                      struct slab_header,
                      node,
                      &temp_slab_pool[order].partial_slab_list) {
        slot = (struct slab_slot_list *)slab->free_list_head;
        while (slot != NULL) {
            current_slot_num++;
            slot = slot->next_free;
...
unsigned long temp_get_free_mem_size_from_slab(void)
{
    int order;
    ...
    for (order = SLAB_MIN_ORDER; order <= SLAB_MAX_ORDER; order++) {
        ...
        slot_num = temp_get_free_slot_number(order);
