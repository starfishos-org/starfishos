...
{
    return page_pair->pages[0].version_number >= page_pair->pages[1].version_number ? 
        &page_pair->pages[0] : &page_pair->pages[1];
}

inline struct ckpt_page* get_second_latest_ckpt_page(struct ckpt_page_pair *page_pair) 
{
    return page_pair->pages[0].version_number >= page_pair->pages[1].version_number ? 
