...
static inline void vmr_ckpt(struct vmregion *target_vmr, struct ckpt_vmregion *ckpt_vmr)
{
...
int vmspace_ckpt(struct vmspace *target_vmspace, struct ckpt_vmspace *ckpt_vmspace)
{
    ...
    int vmr_count = 0;
    for_each_in_list(target_vmr, struct vmregion, list_node, &(target_vmspace->vmr_list)) {
        vmr_count++;
    }
    ...
    if (vmr_count == ckpt_vmspace->vmr_count) {
        ...
        ckpt_vmr_array = kmalloc(sizeof(struct ckpt_vmregion) * vmr_count);
    }

    ckpt_vmspace->user_current_mmap_addr = target_vmspace->user_current_mmap_addr;
    ...
    for_each_in_list(target_vmr, struct vmregion, list_node, &(target_vmspace->vmr_list)) {
        ...
        if ((target_vmr->perm & VMR_WRITE) != 0 
            && !is_external_sync_pmo(target_vmr->pmo) /* do not persist ext sync pmo each ckpt */
            && !(target_vmspace->pte_patch_pool)) {
            /* set page to unwritable */
            set_write_in_pgtbl(target_vmspace, target_vmr->start, target_vmr->size, false);
        }
#endif
        /* ckpt vmr */
...
#ifdef HYBRID_MEM
                /* Now page in pte pool might not be really accessed. 
                * A page might be marked as active and pre memcpy by
                * `process_sub_active_list`(ckpt/hot_page_tracker). 
                * We should check here to see if each page is accessed.
                */
...
static struct vmregion* vmr_restore(struct ckpt_vmregion *ckpt_vmr, struct kvs *obj_map)
{
    struct vmregion *target_vmr;
...
int vmspace_restore(struct object *vm_obj, struct ckpt_object *ckpt_vm_obj, struct kvs* obj_map)
{
    ...
    struct ckpt_vmspace *ckpt_vmspace = (struct ckpt_vmspace *)ckpt_vm_obj->opaque;
    ...
    target_vmspace->user_current_mmap_addr = ckpt_vmspace->user_current_mmap_addr;
    target_vmspace->pcid = ckpt_vmspace->pcid;
    target_vmspace->flags |= VM_FLAG_PRESERVE;
    target_vmspace->pte_patch_pool = create_patch_pool();
#ifdef CHECK_PCTBL
