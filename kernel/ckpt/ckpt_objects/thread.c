...
void* alloc_ckpt_ipc_config(int config_type)
{
    ...
    switch(config_type) {
        case IPC_SERVER_REGISTER_CB:{
            ckpt_ipc_config = kmalloc(sizeof(struct ckpt_ipc_server_register_cb_config));
            break;
        }
        ...
        case IPC_SERVER:{
            ckpt_ipc_config = kmalloc(sizeof(struct ckpt_ipc_server_config));
            break;
        }
        default:
            break;
...
static void thread_ctx_restore(struct ckpt_thread_ctx *ckpt_ctx,struct thread_ctx *target_ctx) 
{
    ...
    if (!target_ctx->sc) {
        target_ctx->sc = kmalloc(sizeof(sched_cont_t));
    ...
    if (ckpt_thread->general_ipc_config != NULL) {
        switch(((struct ipc_config*)ckpt_thread->general_ipc_config)->config_type) {
            case IPC_SERVER_REGISTER_CB:{
                ...
                struct ipc_server_register_cb_config *new_config = kmalloc(sizeof(*new_config));
            ...
            case IPC_SERVER_HANDLER:{
                ...
                struct ipc_server_handler_config *new_config = kmalloc(sizeof(*new_config));
            ...
            case IPC_SERVER:{
                ...
                struct ipc_server_config *new_config = kmalloc(sizeof(*new_config));
