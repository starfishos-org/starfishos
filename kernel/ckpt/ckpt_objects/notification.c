...
int notification_ckpt(struct notification *notifc, struct ckpt_notification *ckpt_notifc,int alloc)
{
    ...
    ckpt_notifc->not_delivered_notifc_count = notifc->not_delivered_notifc_count;
    ...
    ckpt_notifc->waiting_thread_roots = kmalloc(ckpt_notifc->waiting_threads_count * sizeof(struct ckpt_obj_root*));
...
int notification_restore(struct object *notifc_obj, struct ckpt_object *ckpt_notifc_obj, struct kvs *obj_map)
{
    ...
    struct ckpt_notification *ckpt_notifc = (struct ckpt_notification *)ckpt_notifc_obj->opaque;
