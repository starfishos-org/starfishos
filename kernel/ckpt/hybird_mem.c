...
{
    u64 dirty_pages = 0, dram_pages = 0, remove_pages = 0,
        newly_added_pages = 0;
    for (int i = 0; i < PLAT_CPU_NUM; i++) {
        ...
        printk("CPU[%d]: get_pte=%d, get_page_pair=%d, page_copy=%d\n",
               i,
               dram_pages_array[i] ? (get_pte_time[i] / dram_pages_array[i]) :
                                     0,
               dirty_pages_array[i] ?
                       (get_page_pair_time[i] / dirty_pages_array[i]) :
                       0,
               dirty_pages_array[i] ?
                       (page_copy_time[i] / dirty_pages_array[i]) :
                       0);
#endif
    }
...
static inline void move_runtime_nvm_page_to_backup(struct pmobject *pmo,
                                                   void *kva, u64 index)
{
...
static inline void *move_runtime_nvm_page_from_backup(struct pmobject *pmo,
                                                      u64 index)
{
    vaddr_t new_va = 0;
    struct ckpt_obj_root *obj_root;
