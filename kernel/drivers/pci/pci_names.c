...
#include <drivers/pci_names.h>

const char *pci_class_name(unsigned int class_code)
{
    unsigned char base_class = (class_code >> 16) & 0xFF;
    unsigned char sub_class = (class_code >> 8) & 0xFF;
    ...
    case 0x08:
        ...
        case 0x07: return "Generic system peripheral";
...
const char* pci_vendor_name(int vid) {
    if (vid == PCI_VENDOR_ID_REDHAT) return "Red Hat Inc.";
    ...
    if (vid == 0x8086) return "Intel Corporation";
...
const char* pci_device_name(int vid, int did) {
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_NET) return "Virtio Network";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_BLOCK) return "Virtio Block";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_BLLN) return "Virtio Balloon";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_CONS) return "Virtio Console";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_SCSI) return "Virtio SCSI";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_RNG) return "Virtio RNG";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_9P) return "Virtio 9P";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_VIRTIO_VSOCK) return "Virtio VSOCK";
    if (vid == PCI_VENDOR_ID_REDHAT && did == PCI_DEVICE_ID_IVSHMEM) return "QEMU IVSHMEM";
