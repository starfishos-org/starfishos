...
static void parse_hdm_decoder_caps(void *base, struct cxl_hdm *hdm)
{
    ...
    hdm->target_count = FIELD_GET(CXL_HDM_DECODER_TARGET_COUNT_MASK, hdm_cap);
                              ...
                              struct cxl_component_reg_map *map)
{
    ...
    *map = (struct cxl_component_reg_map){0};
    ...
    for (cap = 1; cap <= cap_count; cap++) {
        ...
        rmap = NULL;
        cxl_debug("[CXL] base + cxp * 0x4=%llx, cap_id=%d, offset = %d\n",
                  (u64)(base + cap * 0x4),
                  cap_id,
                  offset);
        ...
        case CXL_CM_CAP_CAP_ID_HDM: {
            ...
            parse_hdm_decoder_caps((void *)register_block, &cxl_hdm);
            enable_hdm_decoder(register_block);
            cxl_decoder_commit(
                    &cxl_hdm, 0, cxl_mem_devs[0].start, cxl_mem_devs[0].size);
            break;
        }
                           ...
                           struct cxl_device_reg_map *map)
{
    ...
    *map = (struct cxl_device_reg_map){0};
    ...
    for (cap = 1; cap <= cap_count; cap++) {
        ...
        cap_id =
                FIELD_GET(CXLDEV_CAP_HDR_CAP_ID_MASK, readl(base + cap * 0x10));
