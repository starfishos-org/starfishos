...
int query_in_pgtbl(void *pgtbl, vaddr_t va, paddr_t *pa, pte_t **entry)
{
    ...
    if (ret < 0)
        ...
        if (pa)
            *pa = virt_to_phys((void *)l2_ptp) + GET_VA_OFFSET_L1(va);
    ...
    if (ret < 0)
        ...
        if (pa)
            *pa = virt_to_phys((void *)l3_ptp) + GET_VA_OFFSET_L2(va);
...
void free_page_table(void *pgtbl)
{
    ...
    for (i = 0; i < PTP_ENTRIES - 2; ++i) {
        l0_pte = &l0_ptp->ent[i];
        if (IS_PTE_INVALID(l0_pte->pteval))
            continue;
        ...
        for (j = 0; j < PTP_ENTRIES; ++j) {
            l1_pte = &l1_ptp->ent[j];
            if (IS_PTE_INVALID(l1_pte->pteval))
                continue;
            ...
            for (k = 0; k < PTP_ENTRIES; ++k) {
                l2_pte = &l2_ptp->ent[k];
                if (IS_PTE_INVALID(l2_pte->pteval))
                    continue;
...
/*
 * For supportting COW:
 *	- scan the page table and modify the writable bit in the PTEs if they
 *exist
 */
                       ...
                       bool flag)
{
    ...
    while (total_page_cnt > 0) {
        ...
        ret = get_next_ptp(l0_ptp, 0, va, &l1_ptp, &pte, false);
        if (ret == -ENOMAPPING) {
            tmp_page_cnt = L0_PER_ENTRY_PAGES
                           - (va >> PAGE_SHIFT & ((1 << (3 * PAGE_ORDER)) - 1));
        ...
        if (ret == -ENOMAPPING) {
            tmp_page_cnt =
                    L1_PER_ENTRY_PAGES - GET_VA_OFFSET_L1(va) / PAGE_SIZE;
            total_page_cnt -= tmp_page_cnt;
            va += tmp_page_cnt * PAGE_SIZE;
            continue;
        }
        ...
        if (ret == -ENOMAPPING) {
            tmp_page_cnt =
                    L2_PER_ENTRY_PAGES - GET_VA_OFFSET_L2(va) / PAGE_SIZE;
            total_page_cnt -= tmp_page_cnt;
            va += tmp_page_cnt * PAGE_SIZE;
            continue;
        }
...
/* This function is used under two conditions: page fault and checkpoint.
        ...
        But when doing checkpoint, the other cores are waiting for the working
   core, thus cannot handle the IPI. Under this condition, we cannot call
        flush_tlb_local_and_remote() here.
        After all, when handling page fault, after calling current function, the
   caller should then flush local and remote TLB. And when the checkpoint is
   done, all cores should then flush its own TLB.
*/
// flush_tlb_local_and_remote(vmspace, va, len);
#endif
                    ...
                    for (k = 0; k < PTP_ENTRIES; k++) {
                        ...
                        if (entry->pteval & PAGE_PRESENT) {
                            ...
                            for (l = 0; l < PTP_ENTRIES; l++) {
                                ...
                                if (entry->pteval & PAGE_PRESENT
                                    && entry->pteval & PAGE_RW) {
                                    vaddr_t va = ((u64)i << (L0_INDEX_SHIFT))
                                                 | (j << L1_INDEX_SHIFT)
                                                 | (k << L2_INDEX_SHIFT)
                                                 | (l << L3_INDEX_SHIFT);
                                    vaddr_t real_va = GET_NEXT_PTP(entry)
                                                      + GET_VA_OFFSET_L3(va);
                                    if (get_page_type(
                                                virt_to_page((void *)real_va))
                                        != DRAM_CACHED_PAGE) {
                                        printk("%llx, pte:%llx\n", va, entry);
                                        ret = 0;
                                    }
