...
void parse_nvm_map(void *info)
{
	...
	u64 nvmlength, max_paddr, limit_paddr, nvm_size;
	...
	if (nvmmem_map[0][0] >= limit_paddr)
		BUG("NVM region base exceed paddr limitation");

	/* if NVM region exceed max virtual address, resize it */
	if (nvmmem_map[0][1] > limit_paddr) {
		nvm_size = (limit_paddr - nvmmem_map[0][0]) / (128 * SIZE_1G);
		if (nvm_size == 0) {
			BUG("no enough NVM");
		}
