...
static void memcpy_and_flush_tlb_on_remote_machine_msi(struct vmspace *vmspace, mid_t target_mid,
...
static void memcpy_and_flush_tlb_on_remote_machine_polling(struct vmspace *vmspace, mid_t target_mid,
                                                            paddr_t src_pa, paddr_t dst_pa, size_t len,
                                                            vaddr_t fault_va)
{
    mid_t my_id = CUR_MACHINE_ID;
...
/* Unified interface: automatically selects MSI or polling mode based on current configuration */
                                             ...
                                             vaddr_t fault_va)
{
    ...
    if (mode == IVSHMEM_MSG_MODE_MSI) {
        memcpy_and_flush_tlb_on_remote_machine_msi(vmspace, target_mid, src_pa, dst_pa, len, fault_va);
    } else {
        memcpy_and_flush_tlb_on_remote_machine_polling(vmspace, target_mid, src_pa, dst_pa, len, fault_va);
    }
}
#endif
