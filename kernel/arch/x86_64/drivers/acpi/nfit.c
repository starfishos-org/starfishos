...
void parse_nfit(struct acpi_table_nfit *table)
{
    ...
    while ((char *)entry + entry->length <= (char *)table + table_len) {
        ...
        case ACPI_NFIT_TYPE_SYSTEM_ADDRESS: {
            ...
            nvm_region_head[nvm_region_num].base = sys_addr->address;
            nvm_region_head[nvm_region_num].length = sys_addr->length;
        ...
        entry = (struct acpi_nfit_header *)((char *)entry + entry->length);
    }
}
