...
struct acpi_table_facs {
        ...
        u32 firmware_waking_vector; /* 32-bit physical address of the Firmware
                                       Waking Vector */
        u32 global_lock; /* Global Lock for shared hardware resources */
        u32 flags;
        u64 xfirmware_waking_vector; /* 64-bit version of the Firmware Waking
                                        Vector (ACPI 2.0+) */
...
#define ACPI_FACS_64BIT_WAKE \
        (1 << 1) /* 01: 64-bit wake vector supported (ACPI 4.0) */
...
struct acpi_table_fadt {
        ...
        u8 model; /* System Interrupt Model (ACPI 1.0) - not used in ACPI 2.0+
                   */
        u8 preferred_profile; /* Conveys preferred power management profile to
                                 OSPM. */
        ...
        u32 pm1a_event_block; /* 32-bit port address of Power Mgt 1a Event Reg
                                 Blk */
        u32 pm1b_event_block; /* 32-bit port address of Power Mgt 1b Event Reg
                                 Blk */
        ...
        u32 pm1b_control_block; /* 32-bit port address of Power Mgt 1b Control
                                   Reg Blk */
        u32 pm2_control_block; /* 32-bit port address of Power Mgt 2 Control Reg
                                  Blk */
        u32 pm_timer_block; /* 32-bit port address of Power Mgt Timer Ctrl Reg
                               Blk */
        u32 gpe0_block; /* 32-bit port address of General Purpose Event 0 Reg
                           Blk */
        u32 gpe1_block; /* 32-bit port address of General Purpose Event 1 Reg
                           Blk */
        ...
        u8 cst_control; /* Support for the _CST object and C-States change
                           notification */
...
#define ACPI_FADT_LEGACY_DEVICES \
        (1) /* 00: [V2] System has LPC or ISA bus devices */
#define ACPI_FADT_8042 \
        (1 << 1) /* 01: [V3] System has an 8042 controller on port 60/64 */
#define ACPI_FADT_NO_VGA \
        (1 << 2) /* 02: [V4] It is not safe to probe for VGA hardware */
#define ACPI_FADT_NO_MSI                                                   \
        (1 << 3) /* 03: [V4] Message Signaled Interrupts (MSI) must not be \
                    enabled */
#define ACPI_FADT_NO_ASPM \
        (1 << 4) /* 04: [V4] PCIe ASPM control must not be enabled */
#define ACPI_FADT_NO_CMOS_RTC \
...
#define ACPI_FADT_PSCI_USE_HVC                                            \
        (1 << 1) /* 01: [V5+] HVC must be used instead of SMC as the PSCI \
                    conduit */

/* Masks for FADT flags */

#define ACPI_FADT_WBINVD \
        (1) /* 00: [V1] The WBINVD instruction works properly */
#define ACPI_FADT_WBINVD_FLUSH \
        (1 << 1) /* 01: [V1] WBINVD flushes but does not invalidate caches */
#define ACPI_FADT_C1_SUPPORTED \
        (1 << 2) /* 02: [V1] All processors support C1 state */
#define ACPI_FADT_C2_MP_SUPPORTED \
        (1 << 3) /* 03: [V1] C2 state works on MP system */
#define ACPI_FADT_POWER_BUTTON                                           \
        (1 << 4) /* 04: [V1] Power button is handled as a control method \
                    device */
#define ACPI_FADT_SLEEP_BUTTON                                           \
        (1 << 5) /* 05: [V1] Sleep button is handled as a control method \
                    device */
#define ACPI_FADT_FIXED_RTC                                                   \
        (1 << 6) /* 06: [V1] RTC wakeup status is not in fixed register space \
                  */
#define ACPI_FADT_S4_RTC_WAKE \
        (1 << 7) /* 07: [V1] RTC alarm can wake system from S4 */
...
#define ACPI_FADT_SEALED_CASE                                                \
        (1 << 11) /* 11: [V3] No internal expansion capabilities and case is \
                     sealed */
#define ACPI_FADT_HEADLESS                                               \
        (1 << 12) /* 12: [V3] No local video capabilities or local input \
                     devices */
#define ACPI_FADT_SLEEP_TYPE                                                \
        (1 << 13) /* 13: [V3] Must execute native instruction after writing \
                     SLP_TYPx register */
#define ACPI_FADT_PCI_EXPRESS_WAKE                                      \
        (1 << 14) /* 14: [V4] System supports PCIEXP_WAKE (STS/EN) bits \
                     (ACPI 3.0) */
#define ACPI_FADT_PLATFORM_CLOCK                                      \
        (1 << 15) /* 15: [V4] OSPM should use platform-provided timer \
                     (ACPI 3.0) */
#define ACPI_FADT_S4_RTC_VALID                                        \
        (1 << 16) /* 16: [V4] Contents of RTC_STS valid after S4 wake \
                     (ACPI 3.0) */
#define ACPI_FADT_REMOTE_POWER_ON                                       \
        (1 << 17) /* 17: [V4] System is compatible with remote power on \
                     (ACPI 3.0) */
#define ACPI_FADT_APIC_CLUSTER                                       \
        (1 << 18) /* 18: [V4] All local APICs must use cluster model \
                     (ACPI 3.0) */
#define ACPI_FADT_APIC_PHYSICAL                                            \
        (1 << 19) /* 19: [V4] All local xAPICs must use physical dest mode \
                     (ACPI 3.0) */
#define ACPI_FADT_HW_REDUCED \
        (1 << 20) /* 20: [V5] ACPI hardware is not implemented (ACPI 5.0) */
#define ACPI_FADT_LOW_POWER_S0                                             \
        (1 << 21) /* 21: [V5] S0 power savings are equal or better than S3 \
                     (ACPI 5.0) */
...
#define ACPI_TABLE_ORIGIN_EXTERNAL_VIRTUAL \
        (0) /* Virtual address, external maintained */
#define ACPI_TABLE_ORIGIN_INTERNAL_PHYSICAL \
        (1) /* Physical address, internally mapped */
