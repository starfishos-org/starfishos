...
static void *find_table_by_sig(char *sig)
{
    ...
    entries = xsdt ? (xsdt->header.length - sizeof(xsdt->header))
                              / ACPI_XSDT_ENTRY_SIZE :
                     (rsdt->header.length - sizeof(rsdt->header))
                              / ACPI_RSDT_ENTRY_SIZE;

    for (i = 0; i < entries; i++) {
        h = xsdt ? (struct acpi_table_header *)(xsdt->table_offset_entry[i]) :
...
void parse_acpi_info(void *info)
{
    ...
    if (rsdp->revision == 0) {
        ...
        /* ACPI version >= 2.0, use extended rsdp and xsdt */
        ...
        kinfo("[ACPI INFO] ACPI version >= 2.0, use extended RSDP and XSDT\n");
        h = (struct acpi_table_header *)phys_to_virt(
                rsdp->xsdt_physical_address);
        xsdt = (struct acpi_table_xsdt *)h;
    } else {
        BUG("[ACPI] Unsupported RSDP revision\n");
    }
