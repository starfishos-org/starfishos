...
struct acpi_cdat_dslbis {
    u8 handle;
    u8 flags; /* If Handle matches a DSMAS handle, the definition of this
               * field matches Flags field in HMAT System Locality Latency
               */
    u8 data_type;
    u8 reserved;
    u64 entry_base_unit;
...
struct acpi_csrt_shared_info {
    ...
    /* Resource descriptors immediately follow (Length = Group length -
     * shared_info_length) */
};
...
struct acpi_dbg2_device {
     ...
     * Structure format. address_size (required) - Array of u32 sizes
     * corresponding to each base_address register. Namepath    (required) -
     * Null terminated string. Single dot if not supported. oem_data
     * (optional) - Length is oem_data_length.
     */
};
