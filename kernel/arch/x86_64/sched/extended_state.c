...
static void xsaveopt(void *register_state, u64 feature_mask)
{
    __asm__ volatile("xsaveopt %0"
                     : "+m"(*(u8 *)register_state)
                     : "d"((u32)(feature_mask >> 32)), "a"((u32)feature_mask)
                     : "memory");
}
...
void save_and_release_fpu(struct thread *thread, u32 cpuid)
{
    ...
    /*
     * @thread will be migrated to other CPUs, and it is the fpu_owner of local
     * CPU. So, here to save its fpu states.
     *
    ...
    if (fpu_owner == thread) {
        ...
        dsm_debug(
                "thread %p (cpuid=%d, aff=%d, fpu_owner=%d) is not the fpu_owner of current cpu %d\n",
                thread,
